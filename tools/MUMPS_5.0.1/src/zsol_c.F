...
C
      SUBROUTINE ZMUMPS_SOL_C(root, N, A, LA, IW, LIW, W, LWC, 
     ...
     &              LOCAL_N
      ...
      EXTERNAL ZMUMPS_SOL_R, ZMUMPS_SOL_S
...
99992 FORMAT (//' LEAVING SOLVE (MPI41C) WITH')
...
#if defined(RHSCOMP_BYROWS)
      COMPLEX(kind=8), intent(in) :: RHSCOMP(NCOL_RHSCOMP, LRHSCOMP)
#else
     ...
     &  TYPE_PARAL, LCWORK, KEEP(247), NRHS
      ...
      IF (ONE_PACK .EQ. no .AND. I_AM_SLAVE) THEN
          WRITE(*,*)
     &    "Internal error 1 in ZMUMPS_GATHER_SOLUTION:",
     ...
     &          NSLAVES)) THEN
              ...
              IF (MYID .EQ. MASTER) THEN
                   N2RECV=N2RECV-NPIV
                   IF (NPIV.GT.0) 
     &             CALL ZMUMPS_NPIV_BLOCK_ADD ( ONE_PACK, .TRUE. )
              ELSE
                   IF (NPIV.GT.0) 
     &             CALL ZMUMPS_NPIV_BLOCK_ADD ( ONE_PACK, .FALSE.)
              ENDIF
          ENDIF
        ENDDO
     ...
     &                   NPIV, 1, MPI_INTEGER, COMM, IERR)
      ...
      RETURN
      CONTAINS
        SUBROUTINE ZMUMPS_NPIV_BLOCK_ADD ( ONE_PACK, ON_MASTER )
...
#if defined(RHSCOMP_BYROWS)
      COMPLEX(kind=8), intent(in) :: RHSCOMP (NRHSCOMP_COL,LRHSCOMP)
#else
...
#if defined(RHSCOMP_BYROWS)
      COMPLEX(kind=8), intent(out) :: RHSCOMP(NCOL_RHSCOMP, LRHSCOMP)
#else
     ...
     &                 ScatterRhsR, COMM, STATUS, IERR )
      ...
      SUBROUTINE ZMUMPS_BUILD_POSINRHSCOMP
 ...
 500  NBENT_RHSCOMP = IPOSINRHSCOMP - 1
     ...
     &     NBENT_RHSCOMP = max(NBENT_RHSCOMP, IPOSINRHSCOMP_COL-1)
      RETURN
      END SUBROUTINE ZMUMPS_BUILD_POSINRHSCOMP
      SUBROUTINE ZMUMPS_BUILD_POSINRHSCOMP_AM1
     &           (NSLAVES, N, MYID_NODES,
     &           PTRIST, DAD,
     &           KEEP,KEEP8, PROCNODE_STEPS, IW, LIW, STEP,
     &           POSINRHSCOMP_ROW, POSINRHSCOMP_COL,
     ...
     &            = POSINRHSCOMP_COL(IW(JCOL+JJ)) + N              
      ...
      RETURN
      END SUBROUTINE ZMUMPS_BUILD_POSINRHSCOMP_AM1
