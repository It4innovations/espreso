...
C
     ...
     &         KEEP(IXSZ), COMP, DKEEP(97),MYID)
          IF (LRLU .NE. LRLUS) THEN
           IF (LPOK) THEN
            WRITE(LP, * ) 'PB compress ZMUMPS_FAC_ASM_NIV1_ELT'
      ...
      CALL ZMUMPS_LOAD_MEM_UPDATE(SSARBR,.FALSE.,LA-LRLUS,0_8,LAELL8,
     &    KEEP,KEEP8,
     &     LRLUS)
#if ! defined(ALLOW_NON_INIT)
  ...
  205     IF (LEVEL1) THEN 
           ...
           ENDIF
             CALL ZMUMPS_FREE_BLOCK_CB(SSARBR, MYID, N, ISTCHK,
     &       IACHK,
  ...
  280 CONTINUE
      IF (LPOK) THEN
        WRITE( LP, * )
     &' FAILURE, WORKSPACE TOO SMALL DURING ZMUMPS_ASM_NIV1_ELT'
      ENDIF
      INFO(1) = -9
  ...
  300 CONTINUE
     ...
     &                 ' DYNAMIC ALLOCATION DURING ZMUMPS_ASM_NIV1_ELT'
      ...
      INFO(1)   = -13
      INFO(2)  = NUMSTK 
  490 CALL  ZMUMPS_BDC_ERROR( MYID, SLAVEF, COMM )
  500 CONTINUE
      RETURN
  ...
  170       CONTINUE
          ...
          ELSE
            ...
            ENDIF
            IF (LCB .GT. 0) THEN
              CALL ZMUMPS_LDLT_ASM_NIV12(A, LA, A(IACHK),
     &           POSELT, LDAFS, NASS1,
     &           LDA_SON, LCB,
  ...
  280 CONTINUE
      IF (LPOK) THEN
        WRITE( LP, * )
     &  ' FAILURE, WORKSPACE TOO SMALL DURING ZMUMPS_ASM_NIV2_ELT'
      ENDIF
      INFO(1) = -9
      CALL MUMPS_SET_IERROR(LAELL8-LRLUS, INFO(2))
      GOTO 490
