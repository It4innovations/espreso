...
C
       SUBROUTINE ZMUMPS_FAC_A(N, NZ, NSCA, 
 ...
 10     CONTINUE
        ...
        IWNOR = 1
          IF (NSCA.EQ.1) THEN
            CALL ZMUMPS_FAC_V(N,NZ,ASPK,IRN,ICN,
     &        COLSCA,ROWSCA,MPG)
          ELSEIF (NSCA.EQ.3) THEN
            CALL ZMUMPS_FAC_Y(N,NZ,ASPK,IRN,ICN,WK_REAL(IWNOR),
     &      COLSCA, MPG)
          ELSEIF (NSCA.EQ.4) THEN
            CALL ZMUMPS_ROWCOL(N,NZ,IRN,ICN,ASPK,
 ...
 500  CONTINUE
      RETURN
      END SUBROUTINE ZMUMPS_FAC_A
      SUBROUTINE ZMUMPS_ROWCOL(N,NZ,IRN,ICN,VAL,
 ...
 110   CONTINUE
     ...
     &  WRITE(MPRINT,*) ' END OF SCALING BY MAX IN ROW AND COL'
      RETURN
      END SUBROUTINE ZMUMPS_ROWCOL
      SUBROUTINE ZMUMPS_FAC_Y(N,NZ,VAL,IRN,ICN,
     &       CNOR,COLSCA,MPRINT)
      INTEGER N,NZ
      COMPLEX(kind=8) VAL(NZ)
 ...
 215   CONTINUE
      ...
      RETURN
      END SUBROUTINE ZMUMPS_FAC_Y
      SUBROUTINE ZMUMPS_FAC_V(N,NZ,VAL,IRN,ICN,
 ...
 110   CONTINUE
      ...
      RETURN
      END SUBROUTINE ZMUMPS_FAC_V
      SUBROUTINE ZMUMPS_FAC_X(NSCA,N,NZ,IRN,ICN,VAL,
     &    RNOR,ROWSCA,MPRINT)
      INTEGER N, NZ, NSCA
      INTEGER IRN(NZ), ICN(NZ)
      COMPLEX(kind=8) VAL(NZ)
