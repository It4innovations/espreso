...
C
      SUBROUTINE ZMUMPS_MTRANSI(ICNTL,CNTL)
      IMPLICIT NONE
   ...
   20 CONTINUE
      RETURN
      END SUBROUTINE ZMUMPS_MTRANSI
      SUBROUTINE ZMUMPS_MTRANSB
     ...
     &        K,KK,KK1,KK2,I0,UP,LOW
      ...
      INTRINSIC abs,min
      EXTERNAL ZMUMPS_MTRANSD, ZMUMPS_MTRANSE,
     &         ZMUMPS_MTRANSF, ZMUMPS_MTRANSX
      RLX = D(1)
      NUM = 0
      BV = RINF
      DO 10 K = 1,N
        ...
        DO 115 K = IP(J),IP(J+1)-1
          ...
          ELSE
            ...
            ELSE
              QLEN = QLEN + 1
              L(I) = QLEN
              CALL ZMUMPS_MTRANSD(I,M,Q,D,L,1)
            ENDIF
            JJ = IPERM(I)
            PR(JJ) = J
          ENDIF
  115   CONTINUE
        DO 150 JDUM = 1,NUM
          IF (LOW.EQ.UP) THEN
            ...
            TBV = BV * (ONE-RLX)
            DO 152 IDUM = 1,M
              CALL ZMUMPS_MTRANSE(QLEN,M,Q,D,L,1)
              L(I) = 0
              LOW = LOW - 1
              Q(LOW) = I
            ...
            ELSE
              ...
              IF (DNEW.GE.TBV) THEN
                IF (DI.NE.MINONE) THEN
                  CALL ZMUMPS_MTRANSF(L(I),QLEN,M,Q,D,L,1)
                ENDIF
                L(I) = 0
                LOW = LOW - 1
                Q(LOW) = I
 ...
 1000 IF (M.EQ.N .and. NUM.EQ.N) GO TO 2000
      CALL ZMUMPS_MTRANSX(M,N,IPERM,L,JPERM)
 2000 RETURN
      END SUBROUTINE ZMUMPS_MTRANSB
      SUBROUTINE ZMUMPS_MTRANSD(I,N,Q,D,L,IWAY)
      IMPLICIT NONE
      INTEGER I,N,IWAY
      INTEGER Q(N),L(N)
   ...
   20 Q(POS) = I
      L(I) = POS
      RETURN
      END SUBROUTINE ZMUMPS_MTRANSD
      SUBROUTINE ZMUMPS_MTRANSE(QLEN,N,Q,D,L,IWAY)
      IMPLICIT NONE
      INTEGER QLEN,N,IWAY
      INTEGER Q(N),L(N)
      DOUBLE PRECISION D(N)
   ...
   20 Q(POS) = I
      L(I) = POS
      RETURN
      END SUBROUTINE ZMUMPS_MTRANSE
      SUBROUTINE ZMUMPS_MTRANSF(POS0,QLEN,N,Q,D,L,IWAY)
      IMPLICIT NONE
   ...
   40 Q(POS) = I
      L(I) = POS
      RETURN
      END SUBROUTINE ZMUMPS_MTRANSF
      SUBROUTINE ZMUMPS_MTRANSQ(IP,LENL,LENH,W,WLEN,A,NVAL,VAL)
      IMPLICIT NONE
      INTEGER WLEN,NVAL
  ...
  100 CONTINUE
      RETURN
      END SUBROUTINE ZMUMPS_MTRANSR
      SUBROUTINE ZMUMPS_MTRANSS(M,N,NE,IP,IRN,A,IPERM,NUMX,
     ...
     &        W(N),LEN(N),LENL(N),LENH(N),FC(N),IW(M),IW4(3*N+M)
      ...
      EXTERNAL ZMUMPS_MTRANSQ,ZMUMPS_MTRANSU,ZMUMPS_MTRANSX
      DO 20 J = 1,N
        FC(J) = J
        LEN(J) = IP(J+1) - IP(J)
   20 CONTINUE
   ...
   21 CONTINUE
      ...
      MOD = 1
      NUMX = 0
      CALL ZMUMPS_MTRANSU(CNT,MOD,M,N,IRN,NE,IP,LEN,FC,IW,NUMX,N,
   ...
   50     CONTINUE
          DO 80 IDUM2 = 1,NE
            ...
            CALL ZMUMPS_MTRANSQ(IP,LENL,LEN,W,WLEN,A,NVAL,BVAL)
   ...
   81     MOD = 1
        ELSE
          BMAX = BVAL
          IF (BMAX-BMIN .LE. RLX) GO TO 1000
          CALL ZMUMPS_MTRANSQ(IP,LEN,LENH,W,WLEN,A,NVAL,BVAL)
   ...
   88     MOD = 0
        ENDIF
        CNT = CNT + 1
        CALL ZMUMPS_MTRANSU(CNT,MOD,M,N,IRN,NE,IP,LEN,FC,IW,NUM,NUMX,
     &              IW4(1),IW4(N+1),IW4(2*N+1),IW4(2*N+M+1))
   90 CONTINUE 
 1000 IF (M.EQ.N .and. NUMX.EQ.N) GO TO 2000
      CALL ZMUMPS_MTRANSX(M,N,IPERM,IW,W)
 2000 RETURN
      END SUBROUTINE ZMUMPS_MTRANSS
      SUBROUTINE ZMUMPS_MTRANSU
  ...
  110 CONTINUE
      RETURN
      END SUBROUTINE ZMUMPS_MTRANSU
      SUBROUTINE ZMUMPS_MTRANSW(M,N,NE,IP,IRN,A,IPERM,NUM,
  ...
  115   CONTINUE
        ...
        DO 120 KK = 1,Q0
          ...
          ELSE
            QLEN = QLEN + 1
            L(I) = QLEN
            CALL ZMUMPS_MTRANSD(I,M,Q,D,L,2)
          ENDIF
          JJ = IPERM(I)
          OUT(JJ) = K
          PR(JJ) = J
  ...
  152       CALL ZMUMPS_MTRANSE(QLEN,M,Q,D,L,2)
            LOW = LOW - 1
            Q(LOW) = I
            L(I) = LOW
            IF (QLEN.EQ.0) GO TO 153
          ...
          DO 155 K = K0,K1
            ...
            ELSE
              ...
              IF (DNEW.LE.DMIN) THEN
                IF (L(I).NE.0) THEN
                  CALL ZMUMPS_MTRANSF(L(I),QLEN,M,Q,D,L,2)
                ENDIF
                LOW = LOW - 1
                Q(LOW) = I
                L(I) = LOW
 ...
 1201 CONTINUE
      ...
      CALL ZMUMPS_MTRANSX(M,N,IPERM,L,JPERM)
 2000 RETURN
      END SUBROUTINE ZMUMPS_MTRANSW
      SUBROUTINE ZMUMPS_MTRANSZ
 ...
 1000 CONTINUE
      IF (M.EQ.N .and. NUM.EQ.N) GO TO 2000
      CALL ZMUMPS_MTRANSX(M,N,IPERM,CV,ARP)
 2000 RETURN
      END SUBROUTINE ZMUMPS_MTRANSZ
      SUBROUTINE ZMUMPS_MTRANSX(M,N,IPERM,RW,CW)
      IMPLICIT NONE
      INTEGER M,N
