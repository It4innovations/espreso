...
C
      SUBROUTINE ZMUMPS_INI_DRIVER( id )
...
C     Module needs to know the size of an INTEGER
      CALL ZMUMPS_BUF_INIT( id%KEEP( 34 ), id%KEEP(35) )
C
C     Reception buffer initialized to zero
      NULLIFY(id%BUFR)
C
      id%MAXIS1 = 0
C
...
C
      ...
      NULLIFY(id%JCN)
      NULLIFY(id%A)
      id%NZ_loc = 0 ! will be tested at the beginning of the facto
      NULLIFY(id%IRN_loc)
      NULLIFY(id%JCN_loc)
      NULLIFY(id%A_loc)
      NULLIFY(id%MAPPING)
...
C
      CALL ZMUMPS_RR_INIT_POINTERS(id)
C     Architecture data
      NULLIFY(id%MEM_DIST)
C     Must be nullified because of routine
C     ZMUMPS_SIZE_IN_STRUCT
