...
C
     ...
     &        TAB_POS_IN_PERE(SLAVEF+2,max(1,KEEP(56)))
      ...
      LOGICAL LAST_CALL
      INTEGER LRELAY_INFO
      COMPLEX(kind=8), DIMENSION(:), ALLOCATABLE :: DYN_BLOCFACTO
      ...
      IF ( LRLU .LT. LAELL .OR. IWPOS + NPIV - 1 .GT. IWPOSCB ) THEN
        IF ( LRLUS .LT. LAELL ) THEN
          ...
          IF (ICNTL(1).GT.0 .AND. ICNTL(4).GE.1) THEN
            LP=ICNTL(1)
            WRITE(LP,*)
     &" FAILURE, WORKSPACE TOO SMALL DURING ZMUMPS_PROCESS_BLOCFACTO"
          ENDIF
          GOTO 700
        END IF
     ...
     &      KEEP(IXSZ),COMP,DKEEP(97),MYID)
        ...
        WRITE(*,*) 'PB compress ZMUMPS_PROCESS_BLOCFACTO, LRLU,LRLUS='
     &       ,LRLU,LRLUS
             IFLAG = -9
             CALL MUMPS_SET_IERROR( LAELL-LRLUS, IERROR )
             GOTO 700
        ...
        IF ( IWPOS + NPIV - 1 .GT. IWPOSCB ) THEN
          IF (ICNTL(1).GT.0 .AND. ICNTL(4).GE.1) THEN
            LP=ICNTL(1)
            WRITE(LP,*)
     &" FAILURE IN INTEGER ALLOCATION DURING ZMUMPS_PROCESS_BLOCFACTO"
          ENDIF
          IFLAG = -8
 ...
 700  CONTINUE
      ...
      RETURN
      END SUBROUTINE ZMUMPS_PROCESS_BLOCFACTO
