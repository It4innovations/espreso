...
C
      ...
      CONTAINS
      SUBROUTINE ZMUMPS_FAC_PAR(N,IW,LIW,A,LA,
     ...
     &             )
      ...
      USE ZMUMPS_FACTO_NIV1_M
      USE ZMUMPS_FACTO_NIV2_M
      IMPLICIT NONE
 ...
 20   CONTINUE
     ...
     &       )
      CALL ZMUMPS_LOAD_RECV_MSGS(COMM_LOAD, KEEP)
 ...
 30   CONTINUE
     ...
     &                   .AND. KEEP(60).ne.0 )
       IF ( KEEP(50) .eq. 0 ) THEN
         CALL  ZMUMPS_FACTO_NIV2( COMM_LOAD, ASS_IRECV,
 ...
 200  CONTINUE
     ...
     &    CALL ZMUMPS_LOAD_UPDATE(1,.FALSE.,FLOP1,KEEP,KEEP8)
      ...
      GO TO 20
 635  CONTINUE
      CALL ZMUMPS_BDC_ERROR( MYID_NODES, SLAVEF, COMM_NODES )
 640  CONTINUE
        CALL ZMUMPS_MPI_CANCEL( INFO(1),
     &       ASS_IRECV, BUFR, LBUFR,
     &       LBUFR_BYTES,
     &       COMM_NODES,
     &       MYID_NODES, SLAVEF)
       CALL ZMUMPS_CLEAN_PENDING( INFO(1),
     &      BUFR, LBUFR,
     &      LBUFR_BYTES,
     ...
     &                   'of size before ZMUMPS_FACTO_ROOT',  LBUFRX
                   ...
                   IS_BUFRX_ALLOCATED = .FALSE.
               ENDIF
               CALL ZMUMPS_FACTO_ROOT( MYID_NODES, MASTER_ROOT,
     &               root, N, KEEP(38),
