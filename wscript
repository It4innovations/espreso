...
def configure(ctx):
    ...
    def setup_wrapper(wrapper_name, requested_lib, all_libs):
        ...
        if requested_lib != "empty":
            ctx.env.append_unique("DEFINES_WRAPPERS", [ define_string ] )
...
def build(ctx):
    ...
    ctx.build_espreso(ctx.path.ant_glob('src/math/**/*.cpp'), "math", [ "WRAPPERS", "BLAS", "LAPACK", "MKL", "SUITESPARSE", "MUMPS", "STRUMPACK", "PASTIX", "SUPERLU_DIST" ])
...
#    ctx.build_espreso(ctx.path.ant_glob('src/wrappers/simd/**/*.cpp'), "simd")
    ctx.build_espreso(ctx.path.ant_glob('src/wrappers/blas/**/*.cpp'), "wblas", [ "WRAPPERS", "BLAS", "MKL" ])
    ctx.build_espreso(ctx.path.ant_glob('src/wrappers/lapack/**/*.cpp'), "wlapack", [ "WRAPPERS", "LAPACK", "MKL" ])
    ctx.build_espreso(ctx.path.ant_glob('src/wrappers/mkl/**/*.cpp'), "wmkl", [ "WRAPPERS", "MKL" ])
    ...
    if ctx.env.HAVE_CUDA:
        ctx.build_espreso(ctx.path.ant_glob('src/wrappers/cuda/**/*.(cu|cpp)'), "wcuda", [ "WRAPPERS", "CUDA" ])
#     ctx.build_espreso(ctx.path.ant_glob('src/wrappers/hypre/**/*.cpp'), "whypre", [ "HYPRE" ])
    ctx.build_espreso(ctx.path.ant_glob('src/wrappers/mklpdss/**/*.cpp'), "wmklpdss", [ "WRAPPERS", "MKLPDSS", "MKL" ])
...
#     ctx.build_espreso(ctx.path.ant_glob('src/wrappers/wsmp/**/*.cpp'), "wwsmp", [ "WSMP" ])
    ...
    ctx.build_espreso(ctx.path.ant_glob('src/wrappers/superlu_dist/**/*.cpp'), "wsuperlu_dist", [ "WRAPPERS", "SUPERLU_DIST" ])
...
def settings(ctx):
    ...
    ctx.msg("  USED WRAPPERS ------------------------", "--------------------------", color="NORMAL")
    wraper_msg("dnblas")
    wraper_msg("dnsolver")
    wraper_msg("lapack")
    wraper_msg("spblas")
...
def recurse(ctx):
    ...
    ctx.recurse("src/wrappers/superlu_dist")
