...
C
      MODULE DMUMPS_FACTO_NIV2_M
      CONTAINS
      SUBROUTINE DMUMPS_FACTO_NIV2( COMM_LOAD, ASS_IRECV, 
     ...
     &        PP_LastPIVRPTRFilled_U
      ...
      IF ( allocok .GT. 0 ) THEN
        WRITE(*,*) MYID,' : DMUMPS_FACTO_NIV2 :failed to allocate ',
     &  NASS, ' integers'
        IFLAG  = -13
        IERROR =NASS
        GO TO 490
  ...
  50      CONTINUE  
     ...
     &              A, LA, POSELT, IFINB)
          ...
          IF (K263.EQ.0) THEN
            NELIM = IEND_BLR - NPIV
            CALL DMUMPS_SEND_FACTORED_BLOCK( COMM_LOAD, ASS_IRECV, 
 ...
 500  CONTINUE
      DEALLOCATE( IPIV )
      RETURN
      END SUBROUTINE DMUMPS_FACTO_NIV2
      END MODULE DMUMPS_FACTO_NIV2_M
