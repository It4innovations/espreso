...
C
      ...
      IPOOL(LPOOL)   = LEAF-1
      RETURN
      END SUBROUTINE ZMUMPS_INIT_POOL_LAST3
      SUBROUTINE ZMUMPS_INSERT_POOL_N
     ...
     &     (( K80 == 2 .OR. K80==3 ) .AND.
     &          ( K47 == 4 ))) THEN
            CALL ZMUMPS_REMOVE_NODE(INODE,1)
         ENDIF
      ENDIF
 ...
 20     CONTINUE
      ...
      POOL(LPOOL - 1) = NBTOP
      RETURN
      END SUBROUTINE ZMUMPS_INSERT_POOL_N
      ...
      NBINSUBTREE = POOL(LPOOL)
      NBTOP       = POOL(LPOOL - 1)
      ZMUMPS_POOL_EMPTY = (NBINSUBTREE + NBTOP == 0)
      RETURN
      END FUNCTION ZMUMPS_POOL_EMPTY
      SUBROUTINE ZMUMPS_EXTRACT_POOL( N, POOL, LPOOL, PROCNODE, SLAVEF,
     ...
     &        ND(KEEP(28))
      EXTERNAL MUMPS_INSSARBR, MUMPS_ROOTSSARBR, ZMUMPS_POOL_EMPTY
      LOGICAL MUMPS_INSSARBR, MUMPS_ROOTSSARBR, ZMUMPS_POOL_EMPTY
      ...
      IF ( KEEP(76) > 6 .OR. KEEP(76) < 0 ) THEN
         WRITE(*,*) "Error 2 in ZMUMPS_EXTRACT_POOL: unknown strategy"
         CALL MUMPS_ABORT()
      ENDIF
      ATOMIC_SUBTREE =  ( KEEP(76) == 1 .OR. KEEP(76) == 3)
      IF ( ZMUMPS_POOL_EMPTY(POOL, LPOOL) ) THEN
         WRITE(*,*) "Error 1 in ZMUMPS_EXTRACT_POOL"
         CALL MUMPS_ABORT()
      ENDIF
      ...
      ELSE
         IF ( INSUBTREE == 1 ) THEN
            IF (NBINSUBTREE == 0) THEN
               WRITE(*,*) "Error 3 in ZMUMPS_EXTRACT_POOL"
               CALL MUMPS_ABORT()
            ENDIF
            LEFT = .TRUE.
         ELSE
 ...
 222  CONTINUE
...
#endif
               CALL ZMUMPS_MEM_NODE_SELECT(INODE,POOL,LPOOL,N,
     ...
     &              PROC_FLAG,MIN_PROC)
         ...
         ELSEIF(KEEP(81).EQ.3)THEN
            ...
            IF((INODE.GE.0).AND.(INODE.LE.N))THEN
               ...
               FLAG_MEM=.FALSE.
               CALL ZMUMPS_LOAD_CHK_MEMCST_POOL(FLAG_MEM)
               IF(FLAG_MEM)THEN
                  CALL ZMUMPS_MEM_NODE_SELECT(INODE,POOL,LPOOL,N,
     &                 STEP,KEEP,KEEP8,
     ...
     &           (INSUBTREE.EQ.0))THEN
               CALL ZMUMPS_LOAD_SET_SBTR_MEM(.TRUE.)
            ENDIF
            INSUBTREE = 1
     ...
     &           SLAVEF)) THEN
            IF((KEEP(47).GE.2.AND.KEEP(81).EQ.1).AND.
     &           (INSUBTREE.EQ.1))THEN
               CALL ZMUMPS_LOAD_SET_SBTR_MEM(.FALSE.)
            ENDIF
            INSUBTREE = 0
         END IF
      ELSE
         IF (NBTOP < 1 ) THEN
            WRITE(*,*) "Error 5 in ZMUMPS_EXTRACT_POOL", NBTOP
            CALL MUMPS_ABORT()
         ENDIF
         INODE = POOL( LPOOL - 2 - NBTOP )
         IF(KEEP(81).EQ.1)THEN
            CALL ZMUMPS_LOAD_POOL_CHECK_MEM
     ...
     &                 SLAVEF)) THEN
         ...
         ENDIF
         IF(KEEP(81).EQ.2)THEN
            CALL ZMUMPS_MEM_NODE_SELECT(INODE,POOL,LPOOL,N,STEP,
     &           KEEP,KEEP8,
     ...
     &                    PROC_FLAG,MIN_PROC)
                     ...
                     ENDIF
                  ELSE
                     CALL ZMUMPS_LOAD_CLEAN_MEMINFO_POOL(INODE)
                  ENDIF
#if ! defined(NOT_ATM_POOL_SPECIAL)
               ENDIF
#endif
 ...
 666     CONTINUE
     ...
     &           ( KEEP(47) == 4 ))) THEN
               CALL ZMUMPS_REMOVE_NODE(INODE,2)
            ENDIF
         ENDIF
         IF ( INODE < 0 ) THEN
            INODE_EFF = -INODE
 ...
 777  CONTINUE
      ...
      RETURN
      END SUBROUTINE ZMUMPS_EXTRACT_POOL
      SUBROUTINE ZMUMPS_MEM_CONS_MNG(INODE,POOL,LPOOL,N,STEP,
     &     KEEP,KEEP8,
      ...
      IF((INODE.GT.0).AND.(INODE.LE.N))THEN
         ...
         DO I=NBTOP,1,-1
            IF(NODE_TO_EXTRACT.LT.0)THEN
               ...
               CALL ZMUMPS_LOAD_COMP_MAXMEM_POOL(NODE_TO_EXTRACT,
     &                                       TMP_COST,PROC)
               MIN_COST=TMP_COST
               MIN_PROC=PROC
            ELSE
               CALL ZMUMPS_LOAD_COMP_MAXMEM_POOL(POOL(LPOOL-2-I),
     &                                       TMP_COST,PROC)
         ...
         IF((KEEP(47).EQ.4).AND.(NBINSUBTREE.NE.0))THEN
            CALL ZMUMPS_CHECK_SBTR_COST(NBINSUBTREE,INSUBTREE,NBTOP,
     &           MIN_COST,SBTR)
            IF(SBTR)THEN
     ...
     &           to save My friend'
      ...
      ENDIF
#endif
      END SUBROUTINE ZMUMPS_MEM_CONS_MNG
      SUBROUTINE ZMUMPS_MEM_NODE_SELECT(INODE,POOL,LPOOL,N,STEP,
     &     KEEP,KEEP8,
     &     PROCNODE,SLAVEF,MYID,SBTR_FLAG,PROC_FLAG,MIN_PROC)
      USE ZMUMPS_LOAD
      IMPLICIT NONE
      ...
      SBTR_FLAG=.FALSE.
      PROC_FLAG=.FALSE.
      CALL ZMUMPS_MEM_CONS_MNG(INODE,POOL,LPOOL,N,STEP,KEEP,KEEP8,
     &     PROCNODE,SLAVEF,MYID,SBTR_FLAG,PROC_FLAG,MIN_PROC)
      IF(SBTR_FLAG)THEN
         RETURN
      ENDIF
      ...
      IF(.NOT.PROC_FLAG)THEN
         NODE_TO_EXTRACT=INODE
         IF((INODE.GE.0).AND.(INODE.LE.N))THEN
            CALL ZMUMPS_FIND_BEST_NODE_FOR_MEM(MIN_PROC,POOL,
     &           LPOOL,INODE)
            IF(MUMPS_INSSARBR(PROCNODE(STEP(INODE)),
     &           SLAVEF))THEN
 ...
 452     CONTINUE
         ...
         POOL(LPOOL-2-NBTOP)=INODE
      ENDIF
      END SUBROUTINE ZMUMPS_MEM_NODE_SELECT
      SUBROUTINE ZMUMPS_GET_INODE_FROM_POOL
     ...
     &             INODE, STRATEGIE )
      ...
      INTEGER, INTENT(OUT) :: INODE
         ...
         INODE = IPOOL( LEAF )
      RETURN
      END SUBROUTINE ZMUMPS_GET_INODE_FROM_POOL
