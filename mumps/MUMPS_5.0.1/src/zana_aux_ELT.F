...
C
      SUBROUTINE ZMUMPS_ANA_F_ELT(N, NELT, ELTPTR, ELTVAR, LIW,
     &            IKEEP, PTRAR,
...
#endif
      INTEGER :: OPT_METIS_SIZE,IERR
      INTEGER, DIMENSION(:), ALLOCATABLE :: OPTIONS_METIS
      INTEGER IDUM 
      EXTERNAL MUMPS_ANA_H, ZMUMPS_ANA_G11_ELT, ZMUMPS_ANA_G12_ELT,
     &         ZMUMPS_ANA_G1_ELT, ZMUMPS_ANA_G2_ELT, 
     &         ZMUMPS_ANA_J1_ELT, ZMUMPS_ANA_J2_ELT,
     &         ZMUMPS_ANA_K,
   ...
   10 L1 = 1
...
#endif
      IDUM=0
      CALL ZMUMPS_NODEL(NELT, N, ELTPTR(NELT+1)-1, ELTPTR, ELTVAR,
     &           XNODEL, NODEL, IW(L1), IDUM, ICNTL)
      IF (IORD.NE.1 .AND. IORD .NE. 5) THEN
        IORD = 0
        IF (NOSUPERVAR) THEN
          CALL ZMUMPS_ANA_G1_ELT(N, NZ, NELT, ELTPTR(NELT+1)-1, 
     &              ELTPTR, ELTVAR, XNODEL, NODEL,
     &              PTRAR(1,2), IW(L1))
        ELSE
         CALL ZMUMPS_ANA_G11_ELT(N, NZ, NELT, ELTPTR(NELT+1)-1, 
     &              ELTPTR, ELTVAR, XNODEL, NODEL,
     &              PTRAR(1,2), 4*N+4, IW(L1))
        ...
        ENDIF
        IF (NOSUPERVAR) THEN
         CALL ZMUMPS_ANA_G2_ELT(N, NZ, NELT, ELTPTR(NELT+1)-1, 
     &              ELTPTR, ELTVAR, XNODEL, NODEL,
     &              IW2, LLIW, PTRAR, PTRAR(1,2),
     &              IW(L1), IWFR)
        ELSE
         CALL ZMUMPS_ANA_G12_ELT(N, NZ, NELT, ELTPTR(NELT+1)-1, 
     &              ELTPTR, ELTVAR, XNODEL, NODEL,
     &              IW2, LLIW, PTRAR, PTRAR(1,2),
     &              IW(L1), IWFR)
        ENDIF
        IF (NOSUPERVAR) THEN
         CALL MUMPS_HAMD(N, LLIW, PTRAR, IWFR, PTRAR(1,2), IW2,
     &   IW(L1), IKEEP,
     ...
     &   LISTVAR_SCHUR, SIZE_SCHUR)
         ...
         ELSE
           WRITE(*,*) "Internal error in ZMUMPS_ANA_F_ELT",KEEP(60)
           CALL MUMPS_ABORT()
         ENDIF
        ELSE
         CALL MUMPS_AMD_ELT(N, LLIW, PTRAR, IWFR, PTRAR(1,2), IW2, 
     &   IW(L1), IKEEP, 
     &   IKEEP(1,2), NCMPA, FILS, IKEEP(1,3), IW(L2), PTRAR(1,3))
        ENDIF
      ELSE
     ...
     &              PTRAR(1,2), IW(L1))
         ...
         IF (allocok.GT.0) THEN
           ...
           RETURN
         ENDIF
         CALL ZMUMPS_ANA_G2_ELTNEW(N, NZ, NELT, ELTPTR(NELT+1)-1, 
     &              ELTPTR, ELTVAR, XNODEL, NODEL,
     ...
     &              IW(L1), IWFR)
         ...
         OPTIONS_METIS(18) = 1
#endif
         CALL METIS_NODEND(N, IW(L2), IW2(1),
#if defined(metis4) || defined(parmetis3)
     &        NUMFLAG,
#else
     &        NUMFLAG(1),
     ...
     &    GO TO 40
         ...
         ENDIF
       ENDDO
       CALL ZMUMPS_ANA_J1_ELT(N, NZ, NELT, ELTPTR(NELT+1)-1,
     &             ELTPTR, ELTVAR, XNODEL, NODEL, 
     &             IKEEP, PTRAR(1,2), IW(L1))
       ...
       IF (allocok.GT.0) THEN
         ...
         RETURN
       ENDIF
       CALL ZMUMPS_ANA_J2_ELT(N, NZ, NELT, ELTPTR(NELT+1)-1,
     &             ELTPTR, ELTVAR, XNODEL, NODEL, 
     ...
     &             IW(L1), IWFR)
       ...
       ELSE
         ...
         ENDIF
       ENDIF
       CALL ZMUMPS_ANA_K(N, PTRAR, IW2, LLIW, IWFR, IKEEP, 
     &    IKEEP(1,2), IW(L1),
     &    IW(L2), NCMPA, ITEMP)
      ENDIF
#if defined(OLDDFS)
      CALL ZMUMPS_ANA_L(N, PTRAR, IW(L1), IKEEP, IKEEP(1,2),
     &     IKEEP(1,3),
     &     NFSIZ, INFO(6), FILS, FRERE, PTRAR(1,3), NEMIN, KEEP(60))
#else
      CALL ZMUMPS_ANA_LNEW(N, PTRAR, IW(L1), IKEEP, IKEEP(1,2),
     &     IKEEP(1,3),
     &     NFSIZ, PTRAR(1,2), 
...
99982 FORMAT ('Error in permutation array KEEP   INFO(2)=', I3)
      END SUBROUTINE ZMUMPS_ANA_F_ELT
      SUBROUTINE ZMUMPS_NODEL( NELT, N, NELNOD, XELNOD, ELNOD,
  ...
  100 CONTINUE
      ...
      ENDDO
      RETURN
99999 FORMAT (/'*** Warning message from subroutine ZMUMPS_NODEL ***')
      END SUBROUTINE ZMUMPS_NODEL
      SUBROUTINE ZMUMPS_ANA_G1_ELT(N, NZ, NELT, NELNOD,
     ...
     &        FLAG(N)
      ...
      ENDDO
      RETURN
      END SUBROUTINE ZMUMPS_ANA_G1_ELT
      SUBROUTINE ZMUMPS_ANA_G2_ELTNEW(N, NZ, NELT, NELNOD,
     &  XELNOD, ELNOD, XNODEL, NODEL, 
     ...
     &          IW(LW), FLAG(N)
      ...
      ENDDO
      RETURN
      END SUBROUTINE ZMUMPS_ANA_G2_ELTNEW
      SUBROUTINE ZMUMPS_ANA_G2_ELT(N, NZ, NELT, NELNOD,
     &  XELNOD, ELNOD, XNODEL, NODEL, 
     &  IW, LW, IPE, LEN, FLAG, IWFR)
      IMPLICIT NONE
      INTEGER N,NZ,NELT,NELNOD,LW,IWFR
     ...
     &          IW(LW), FLAG(N)
      ...
      ENDDO
      RETURN
      END SUBROUTINE ZMUMPS_ANA_G2_ELT
      SUBROUTINE ZMUMPS_ANA_J1_ELT(N, NZ, NELT, NELNOD,
     & XELNOD, ELNOD, XNODEL, NODEL, 
     & PERM, LEN, FLAG)
      IMPLICIT NONE
      ...
      ENDDO   
      RETURN  
      END SUBROUTINE ZMUMPS_ANA_J1_ELT
      SUBROUTINE ZMUMPS_ANA_J2_ELT(N, NZ, NELT, NELNOD,
     & XELNOD, ELNOD, XNODEL, NODEL, 
     ...
     &          FLAG(N)
      ...
      ENDDO
      RETURN
      END SUBROUTINE ZMUMPS_ANA_J2_ELT
      SUBROUTINE ZMUMPS_ANA_DIST_ELEMENTS( MYID, SLAVEF, N, 
     ...
     &           KEEP,KEEP8, ICNTL, SYM )
      ...
      DO I = 1, N
        ...
        ITYPE = MUMPS_TYPENODE( PROCNODE(abs(STEP(I))), SLAVEF )
        IRANK = MUMPS_PROCNODE( PROCNODE(abs(STEP(I))), SLAVEF )
     ...
     &       (ITYPE .EQ. 1 .AND. IRANK .EQ. MYID) ) THEN
      ...
      KEEP( 13 ) = IPTRR - 1
      RETURN
      END SUBROUTINE ZMUMPS_ANA_DIST_ELEMENTS
      SUBROUTINE ZMUMPS_ELTPROC( N, NELT, ELTPROC, SLAVEF, PROCNODE )
      IMPLICIT NONE
      INTEGER N, NELT, SLAVEF
      INTEGER PROCNODE( N ), ELTPROC( NELT )
      INTEGER ELT, I, ITYPE, MUMPS_TYPENODE, MUMPS_PROCNODE
      EXTERNAL MUMPS_TYPENODE, MUMPS_PROCNODE
      DO ELT = 1, NELT
          I = ELTPROC(ELT)
          IF ( I .NE. 0) THEN
           ITYPE = MUMPS_TYPENODE(PROCNODE(I),SLAVEF)
           IF (ITYPE.EQ.1) THEN
             ELTPROC(ELT) = MUMPS_PROCNODE(PROCNODE(I),SLAVEF)
      ...
      ENDDO
      RETURN
      END SUBROUTINE ZMUMPS_ELTPROC
      SUBROUTINE ZMUMPS_FRTELT(N, NELT, NELNOD, FRERE, FILS, NA, NE,
 ...
 90   CONTINUE
        IF (III.NE.LEAF) THEN
           ...
           III = III + 1
        ELSE 
           WRITE(6,*) ' ERROR 1 in subroutine ZMUMPS_FRTELT '
           CALL MUMPS_ABORT()
        ENDIF
 95     CONTINUE 
        IN = INODE
 ...
 110    IN = FRERE(IN)
  ...
  115 CONTINUE
      ...
      RETURN
      END SUBROUTINE ZMUMPS_FRTELT
      SUBROUTINE ZMUMPS_ANA_G11_ELT(N, NZ, NELT, NELNOD,
     &  XELNOD, ELNOD, XNODEL, NODEL, 
     &  LEN, LW, IW)
      IMPLICIT NONE
     ...
     &        IW(LW)
      ...
      INTEGER INFO44(6)
      EXTERNAL ZMUMPS_SUPVAR
      LP = 6
      CALL ZMUMPS_SUPVAR(N,NELT,XELNOD(NELT+1)-1,ELNOD,XELNOD,
     &           NSUP,IW(3*N+3+1),3*N+3,IW,LP,INFO44)
      IF (INFO44(1) .LT. 0) THEN
        IF (LP.GE.0) WRITE(LP,*) 
     &     'Error return from ZMUMPS_SUPVAR. INFO(1) = ',INFO44(1) 
      ENDIF
      IW(1:NSUP) = 0
      LEN(1:N) = 0
      DO I = 1,N
      ...
      ENDDO
      RETURN
      END SUBROUTINE ZMUMPS_ANA_G11_ELT
      SUBROUTINE ZMUMPS_ANA_G12_ELT(N, NZ, NELT, NELNOD,
     &  XELNOD, ELNOD, XNODEL, NODEL, 
     &  IW, LW, IPE, LEN, FLAG, IWFR)
      IMPLICIT NONE
      INTEGER N,NZ,NELT,NELNOD,LW,IWFR
     ...
     &          IW(LW), FLAG(N)
      ...
      ENDDO
      RETURN
      END SUBROUTINE ZMUMPS_ANA_G12_ELT
      SUBROUTINE ZMUMPS_SUPVAR(N,NELT,NZ,ELTVAR,ELTPTR,NSUP,SVAR,
     &                 LIW,IW,LP,INFO)
      ...
      VARS = NEW + LIW/3
      FLAG = VARS + LIW/3
      CALL ZMUMPS_SUPVARB(N,NELT,ELTPTR,NZ,ELTVAR,SVAR,NSUP,LIW/3-1,
   ...
   40 INFO(1) = -4
      ...
      END IF
   50 RETURN
 9000 FORMAT (/3X,'Error message from ZMUMPS_SUPVAR: INFO(1) = ',I2)
 9010 FORMAT (3X,'LIW is insufficient. Upper bound on required work',
     &       'space is ',I8)
      END SUBROUTINE ZMUMPS_SUPVAR
      SUBROUTINE ZMUMPS_SUPVARB( N, NELT, ELTPTR, NZ, ELTVAR,
