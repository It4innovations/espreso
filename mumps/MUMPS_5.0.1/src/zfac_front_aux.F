...
C
      MODULE ZMUMPS_FAC_FRONT_AUX_M
      CONTAINS
      SUBROUTINE ZMUMPS_FAC_H(NFRONT,NASS,IW,LIW,A,LA,
 ...
 430  CONTINUE
      RETURN
      END SUBROUTINE ZMUMPS_FAC_H
      SUBROUTINE ZMUMPS_FAC_M(IBEG_BLOCK,
 ...
 541     CONTINUE
     ...
     &              A(LPOS+1_8),NFRONT)
        ENDIF
        RETURN
        END SUBROUTINE ZMUMPS_FAC_M
      SUBROUTINE ZMUMPS_FAC_N(NFRONT,NASS,IW,LIW,A,LA,
 ...
 541    CONTINUE
  ...
  440   CONTINUE
        RETURN
        END SUBROUTINE ZMUMPS_FAC_N
      SUBROUTINE ZMUMPS_FAC_P(A,LA,NFRONT,
     &       NPIV,NASS,POSELT)
      IMPLICIT NONE
      INTEGER(8) :: LA,POSELT
      COMPLEX(kind=8)    A(LA)
     ...
     &          NFRONT,A(LPOS2),NFRONT,ONE,A(LPOS),NFRONT)
      RETURN
      END SUBROUTINE ZMUMPS_FAC_P
      SUBROUTINE ZMUMPS_FAC_P_PANEL(A,LAFAC,NFRONT,
     &      NPIV,NASS, IW, LIWFAC,
     ...
     &          NFRONT,A(LPOS2),NFRONT,ONE,A(LPOS),NFRONT)
        RETURN
        END SUBROUTINE ZMUMPS_FAC_P_PANEL
      SUBROUTINE ZMUMPS_FAC_T(A,LA,NPIVB,NFRONT,
     &                             NPIV,NASS,POSELT)
      IMPLICIT NONE
      INTEGER NPIVB,NASS
      INTEGER(8) :: LA
     ...
     &          NFRONT,A(LPOS1),NFRONT,ONE,A(LPOS),NFRONT)
        RETURN
        END SUBROUTINE ZMUMPS_FAC_T
      SUBROUTINE ZMUMPS_FAC_SQ(IBEG_BLOCK, IEND_BLOCK, NPIV,
 ...
 541     CONTINUE
     ...
     &               A(LPOS),NFRONT,ONE,A(LPOS+1_8),NFRONT)
        ...
        ENDIF
        RETURN
        END SUBROUTINE ZMUMPS_FAC_MQ
      SUBROUTINE ZMUMPS_FAC_FR_UPDATE_CBROWS( INODE,
     ...
     &            IBEG_BLOCK, IFINB, INOPV
      ...
      IF ((NPIV.GT.0).AND.(NEL1.GT.0)) THEN
        IF (KEEP(201).EQ.1) THEN 
         ...
         CALL ZMUMPS_FAC_P_PANEL(A(POSELT), LAFAC, NFRONT, 
 ...
 500  CONTINUE
      RETURN
      END SUBROUTINE ZMUMPS_FAC_FR_UPDATE_CBROWS
      SUBROUTINE ZMUMPS_FAC_I(NFRONT,NASS,LAST_ROW,
     ...
     &       IW, LIW)
        ...
        IF (INOPV .EQ. -1) THEN
           ...
           ELSE IF (KEEP(258) .NE. 0) THEN
              CALL ZMUMPS_UPDATEDETER(A(APOS), DKEEP(6), KEEP(259))
           ENDIF
           IF (KEEP(201).EQ.1) THEN
             IF (KEEP(251).EQ.0) THEN 
               CALL ZMUMPS_STORE_PERMINFO( IW(I_PIVRPTR_L), 
  ...
  430 CONTINUE
      RETURN
      END SUBROUTINE ZMUMPS_FAC_I
      SUBROUTINE ZMUMPS_FAC_I_LDLT 
         ...
         IF ( IS_MAXFROMM_AVAIL ) THEN
            IF ( MAXFROMM > PIVNUL .AND. abs(PIVOT) > TOL) THEN
               IF ( abs(PIVOT) .GT. max(UULOC*MAXFROMM,SEUIL) ) THEN
                 IF (KEEP(258) .NE. 0) THEN
                   CALL ZMUMPS_UPDATEDETER(PIVOT, DKEEP(6), KEEP(259))
                 ENDIF
                 GOTO 415
               ENDIF
            ENDIF
...
!$OMP END PARALLEL DO
     ...
     &          abs(DETPIV)) THEN
           ...
           ENDIF
           IF (KEEP(258) .NE.0 ) THEN
             CALL ZMUMPS_UPDATEDETER(DETPIV, DKEEP(6), KEEP(259))
           ENDIF
           PIVSIZ = 2
           KEEP(103) = KEEP(103)+1
 415       CONTINUE
 ...
 416          CONTINUE
  ...
  420 CONTINUE
      IS_MAXFROMM_AVAIL = .FALSE.
      RETURN
      END SUBROUTINE ZMUMPS_FAC_I_LDLT
      SUBROUTINE ZMUMPS_FAC_MQ_LDLT(IEND_BLOCK,
...
!$OMP      END PARALLEL DO
      ...
      ENDIF
      RETURN
      END SUBROUTINE ZMUMPS_FAC_MQ_LDLT
      SUBROUTINE ZMUMPS_FAC_SQ_LDLT(IBEG_BLOCK,IEND_BLOCK,NPIV,
     ...
     &                     int( IROW - 1,8)
           ...
           CALL zgemm( 'N','N', Block, LAST_VAR - IROW + 1, NPIV_BLOCK,
     &                ALPHA, A( UPOS ), LDA,
     &                A( LPOS ), LDA, ONE, A( APOS ), LDA )
         END DO
#endif
     ...
     &                 int(IEND_BLOCK,8)
       ...
       IF ( POSTPONE_COL_UPDATE ) THEN
         IF (NASS.GT. LAST_VAR) THEN
           CALL zgemm('N', 'N', NEL1, NASS-LAST_VAR, NPIV_BLOCK, ALPHA, 
     &              A(UPOS), LDA, A(LPOS), LDA, ONE, 
     &              A(APOS), LDA)
         ENDIF
       ELSE
         CALL zgemm('N', 'N', NEL1, NFRONT-LAST_VAR, NPIV_BLOCK, ALPHA, 
     &              A(UPOS), LDA, A(LPOS), LDA, ONE, 
     &              A(APOS), LDA)
       END IF
      ENDIF
  500 CONTINUE
      RETURN
      END SUBROUTINE ZMUMPS_FAC_SQ_LDLT
        SUBROUTINE ZMUMPS_SWAP( A, LA, IW, LIW,
     ...
     &        A( IDIAG + int(NASS-IPIV+1,8) * LDA8 ), LDA )
            ...
            ENDIF
        RETURN
        END SUBROUTINE ZMUMPS_SWAP
      SUBROUTINE ZMUMPS_FAC_T_LDLT(NFRONT,NASS,
     &    IW,LIW,A,LA,
     &    LDA,
     ...
     &    )
      ...
      COMPLEX(kind=8)  ONE, ALPHA, BETA, ZERO, VALPIV
      PARAMETER (ONE=(1.0D0,0.0D0), ALPHA=(-1.0D0,0.0D0))
      PARAMETER (ZERO=(0.0D0,0.0D0))
      LDA8 = int(LDA,8)
     ...
     &              A( APOS + LDA8 * int(Block,8) ), LDA )
      ...
      END IF
      RETURN
      END SUBROUTINE ZMUMPS_FAC_T_LDLT
      SUBROUTINE ZMUMPS_STORE_PERMINFO( PIVRPTR, NBPANELS, PIVR, NASS,
     &                                  K, P, LastPanelonDisk,
     &                                  LastPIVRPTRIndexFilled )
      ...
      RETURN
      END SUBROUTINE ZMUMPS_STORE_PERMINFO
      END MODULE ZMUMPS_FAC_FRONT_AUX_M
