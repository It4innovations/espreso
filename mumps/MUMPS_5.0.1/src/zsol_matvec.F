...
C
      SUBROUTINE ZMUMPS_MV_ELT( N, NELT, ELTPTR, ELTVAR, A_ELT,
     &                          X, Y, K50, MTYPE )
      IMPLICIT NONE
C
C  Purpose
...
C             Off diagonal + transpose
     ...
     &           A_ELT( K ) * X( ELTVAR( IELPTR + I ) )
      ...
      END DO
      RETURN
      END SUBROUTINE ZMUMPS_MV_ELT
      SUBROUTINE ZMUMPS_LOC_MV
...
C       matrix was provided (LDLT facto)
     ...
     &        ) CYCLE
      ...
      END IF
      RETURN
      END SUBROUTINE ZMUMPS_LOC_MV
      SUBROUTINE ZMUMPS_MV( N, NZ, IRN, ICN, ASPK, X, Y,
...
C
      INTEGER K, I, J
      COMPLEX(kind=8), DIMENSION(:), ALLOCATABLE :: PX
      COMPLEX(kind=8) ZERO
      INTEGER :: allocok
      PARAMETER( ZERO = (0.0D0,0.0D0) )
      Y = ZERO
...
C       matrix was provided (LDLT facto)
     ...
     &        ) CYCLE
      ...
      DEALLOCATE(PX)
      RETURN
      END SUBROUTINE ZMUMPS_MV
C
C
      SUBROUTINE ZMUMPS_LOC_OMEGA1
     &( N, NZ_loc, IRN_loc, JCN_loc, A_loc, X, Y_loc,
     &  LDLT, MTYPE)
      IMPLICIT NONE
C
...
C       matrix was provided (LDLT facto)
     ...
     &        ) CYCLE
      ...
      END IF
      RETURN
      END SUBROUTINE ZMUMPS_LOC_OMEGA1
