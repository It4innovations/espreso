								...
								const eslocal domains_num)
{
	...
	eslocal local_prim_numbering_offset = 0;
	std::vector <eslocal> local_prim_numbering ( domains_num, local_prim_numbering_offset);

	for (eslocal d = 0; d < domains_num; d++) {
		eslocal dimension = _mesh.getPartNodesCount(d) * Point::size();
	...
	std::map<eslocal,double>::const_iterator itm;
