...
{
    stacktimer::push("gpu_trsm_trirhs_chunk_splitfactor::setup");
    ...
    if(h_L->nrows != h_L->ncols) eslog::error("L is not square\n");
    ...
    h_sub_L_top_sp.set(k_size, k_size, nnz_L_top, cfg.trsm_factor_order, AllocatorHostPinned_new::get_singleton());
    ...
    wss_persistent += utils::round_up(op_trsm.get_wss_persistent(), ator_ws_persistent->get_align());
    ...
    if(d_sub_X_bot.nrows > 0) {
        ...
        wss_persistent += utils::round_up(op_gemm.get_wss_persistent(), ator_ws_persistent->get_align());
...
{
    stacktimer::push("gpu_trsm_trirhs_chunk_splitfactor::preprocess_submit");
...
{
    stacktimer::push("gpu_trsm_trirhs_chunk_splitfactor::perform_submit");
