...
template<typename T, typename I>
void sc_symm_hcsx_ddny_tria<T,I>::set_handles(gpu::mgm::queue q_, gpu::spblas::handle spblas_handle_, gpu::dnblas::handle dnblas_handle_)
{
...
template<typename T, typename I>
void sc_symm_hcsx_ddny_tria<T,I>::set_h_A11_solver(DirectSparseSolver<T,I> * A11_solver_)
{
    if(A11_solver != nullptr) eslog::error("A11_solver is already set\n");
    if(A11_solver_ == nullptr) eslog::error("A11_solver cannot be nullptr\n");
...
template<typename T, typename I>
void sc_symm_hcsx_ddny_tria<T,I>::set_A12(MatrixCsxView_new<T,I> * A12_)
{
    if(A12 != nullptr) eslog::error("matrix A12 is already set\n");
    if(A12_ == nullptr) eslog::error("A12 cannot be nullptr\n");
...
template<typename T, typename I>
void sc_symm_hcsx_ddny_tria<T,I>::set_sc(MatrixDenseView_new<T> * sc_)
{
...
void sc_symm_hcsx_ddny_tria<T,I>::setup()
{
    ...
    if(A11_solver->getMatrixSize() != A12->nrows) eslog::error("incompatible matrices\n");
    ...
    if(solver_factor_uplo == 'U') {
        ...
        Matrix_CSR<T,I,gpu::mgm::Ah> h_factor_old = MatrixCsxView_new<T,I>::to_old(h_factor_row_U);
        A11_solver->getFactorU(h_factor_old, true, false);
    }
    if(solver_factor_uplo == 'L') {
        h_factor_row_L.alloc();
        Matrix_CSR<T,I,gpu::mgm::Ah> h_factor_old = MatrixCsxView_new<T,I>::to_old(h_factor_row_L);
        A11_solver->getFactorL(h_factor_old, true, false);
    }
    ...
    if(order_L == 'R') h_L = &h_L_row;
    if(order_L == 'C') h_L = &h_L_col;

    {
        Permutation<I> h_perm_fillreduce = PermutationView_new<I>::to_old(h_perm_fillreduce);
        A11_solver->getPermutation(h_perm_fillreduce);
    }

    {
        VectorDenseData_new<I> A12_colpivots;
        A12_colpivots.set(A12->ncols, AllocatorCPU_new::get_singleton());
        A12_colpivots.alloc();

        pivots_trails_csx<T,I>::do_all(A12, &A12_colpivots, 'C', 'P', '_');

        h_perm_to_sort_A12_cols.set(A12->ncols, AllocatorCPU_new::get_singleton());
        h_perm_to_sort_A12_cols.alloc();

        sorting_permutation<T,I>::do_all(&A12_colpivots, h_perm_to_sort_A12_cols);
    ...
    op_d_herk.set_mode(math::herk_mode::AhA);
    op_d_herk.setup();
