...
template<typename T, typename I>
void setup_config(typename schur_hcsx_ddny_manual_simple_data<T,I>::config & cfg, size_t size_A11)
{
    using solver_impl_t = typename math::operations::solver_csx<T,I>::implementation_selector;
    using ecf_config = GpuSchurHcsxDdnyManualSimpleConfig;
    const ecf_config & ecf = info::ecf->operations.gpu_schur_hcsx_ddny_manual_simple;
    ...
    switch(ecf.trsm_factor_spdn) {
        case ecf_config::SPDN::AUTO: {
            ...
            if(gpu::mgm::get_implementation() == gpu::mgm::gpu_wrapper_impl::CUDA) {
                ...
                if(gpu::spblas::get_implementation() == gpu::spblas::spblas_wrapper_impl::CUSPARSE_MODERN) {
        ...
        case ecf_config::SPDN::SPARSE: cfg.trsm_factor_spdn = 'S'; break;
        case ecf_config::SPDN::DENSE:  cfg.trsm_factor_spdn = 'D'; break;
    }
...
void schur_hcsx_ddny_manual_simple<T,I>::internal_setup()
{
    ...
    data->op_d_trsm.set_config(data->cfg.trsm_factor_spdn);
    data->op_d_trsm.set_handles(q, handle_spblas, handle_dnblas);
    data->op_d_trsm.set_matrix_h_A(data->h_L_to_use);
    data->op_d_trsm.set_matrix_d_B(&data->d_X_dn);
    ...
    data->op_d_herk = herk_ddnx_ddny<T>::make();
...
template<typename T, typename I>
void schur_hcsx_ddny_manual_simple<T,I>::internal_preprocess_submit()
{
...
template<typename T, typename I>
void schur_hcsx_ddny_manual_simple<T,I>::internal_perform_1_submit()
{
    if(called_set_matrix == '1') {
        data->op_split.perform();
    }
...
template<typename T, typename I>
void schur_hcsx_ddny_manual_simple<T,I>::internal_perform_2_submit()
{
...
template<typename T, typename I>
void schur_hcsx_ddny_manual_simple<T,I>::internal_solve_A11(VectorDenseView_new<T> & rhs, VectorDenseView_new<T> & sol)
{
