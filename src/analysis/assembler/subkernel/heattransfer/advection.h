...
struct AdvectionKernel<nodes, gps, ndim, ThermalConductivityConfiguration::MODEL::DIAGONAL, Physics>: Advection, Physics {
	AdvectionKernel(const Advection &base): Advection(base) {}
...
template <size_t nodes, size_t gps, size_t ndim, class Physics>
struct AdvectionKernel<nodes, gps, ndim, ThermalConductivityConfiguration::MODEL::SYMMETRIC, Physics>: Advection, Physics {
	AdvectionKernel(const Advection &base): Advection(base) {}
	...
	void simd(typename Physics::Element &element)
	{
		...
		for (size_t gp = 0; gp < gps; ++gp) {
			SIMD usq, besq, be[nodes];
			for (size_t d = 0; d < ndim; ++d) {
				SIMD u = element.ecf.advection[gp][d][0] * element.ecf.density[gp] * element.ecf.heatCapacity[gp];
				usq = usq + u * u;
				for (size_t n = 0; n < nodes; ++n) {
					be[n] = be[n] + u * element.dND[gp][n][d];
				}
...
template <size_t nodes, size_t gps, size_t ndim, class Physics>
struct AdvectionKernel<nodes, gps, ndim, ThermalConductivityConfiguration::MODEL::ANISOTROPIC, Physics>: Advection, Physics {
	AdvectionKernel(const Advection &base): Advection(base) {}
