...
struct ElementCoordinates: public ElementOperatorBuilder {
	...
	ElementCoordinates(Assembler &assembler): ElementOperatorBuilder("ELEMENTS COORDINATES"), assembler(assembler)
	{
	...
	bool build() override
	{
		assembler.coords.node.addInput(info::mesh->nodes->coordinates);
		assembler.coords.gp.addInput(assembler.coords.node);
		assembler.coords.node.resize();
		assembler.coords.gp.resize();

		assembler.addParameter(assembler.coords.node);
		assembler.addParameter(assembler.coords.gp);
	...
	void apply(int interval)
	{
		...
		if (info::mesh->dimension == 2) {
			iterate_elements(Coordinates2DToElementNodes(procNodes, assembler.coords.node, interval));
			iterate_elements_gps<typename Assembler::NGP>(FromNodesToGaussPoints<2>(assembler.integration.N, assembler.coords.node, assembler.coords.gp, interval));
		}
		if (info::mesh->dimension == 3) {
			iterate_elements(Coordinates3DToElementNodes(procNodes, assembler.coords.node, interval));
			iterate_elements_gps<typename Assembler::NGP>(FromNodesToGaussPoints<3>(assembler.integration.N, assembler.coords.node, assembler.coords.gp, interval));
		}
	}
};
...
struct BoundaryCoordinates: public BoundaryOperatorBuilder {
	...
	BoundaryCoordinates(Assembler &assembler): BoundaryOperatorBuilder("BOUNDARY COORDINATES"), assembler(assembler)
	{
	...
	bool build() override
	{
		for (size_t r = 0; r < info::mesh->boundaryRegions.size(); ++r) {
			assembler.coords.boundary.node.regions[r].addInput(info::mesh->nodes->coordinates);
			assembler.coords.boundary.gp.regions[r].addInput(assembler.coords.boundary.node.regions[r]);
			assembler.coords.boundary.node.regions[r].isset = true;
			assembler.coords.boundary.gp.regions[r].isset = true;
			assembler.coords.boundary.node.regions[r].resize();
			assembler.coords.boundary.gp.regions[r].resize();

			assembler.addParameter(assembler.coords.boundary.node.regions[r]);
			assembler.addParameter(assembler.coords.boundary.gp.regions[r]);
		}
		return true;
	}
	...
	void apply(int region, int interval)
	{
		...
		if (info::mesh->dimension == 2) {
			iterate_boundary(Coordinates2DToElementNodes(procNodes, assembler.coords.boundary.node.regions[region], interval), region);
			iterate_boundary_gps<Assembler::NGP>(FromNodesToGaussPoints<2>(assembler.integration.boundary.N.regions[region], assembler.coords.boundary.node.regions[region], assembler.coords.boundary.gp.regions[region], interval), region);
		}
		if (info::mesh->dimension == 3) {
			iterate_boundary(Coordinates3DToElementNodes(procNodes, assembler.coords.boundary.node.regions[region], interval), region);
			iterate_boundary_gps<Assembler::NGP>(FromNodesToGaussPoints<3>(assembler.integration.boundary.N.regions[region], assembler.coords.boundary.node.regions[region], assembler.coords.boundary.gp.regions[region], interval), region);
		}
	}
};
