...
struct ElementIntegration: public ElementOperatorBuilder {
	...
	ElementIntegration(Assembler &assembler): ElementOperatorBuilder("ELEMENTS INTEGRATION"), assembler(assembler)
	{
	...
	bool build() override
	{
		assembler.integration.jacobiInversion.addInput(assembler.coords.node);
		assembler.integration.jacobiInversion.addInput(assembler.integration.dN);
		assembler.integration.jacobiInversion.resize();
		assembler.addParameter(assembler.integration.jacobiInversion);

		assembler.integration.jacobiDeterminant.addInput(assembler.coords.node);
		assembler.integration.jacobiDeterminant.addInput(assembler.integration.dN);
		assembler.integration.jacobiDeterminant.resize();
		assembler.addParameter(assembler.integration.jacobiDeterminant);

		assembler.integration.dND.addInput(assembler.coords.node);
		assembler.integration.dND.addInput(assembler.integration.dN);
		assembler.integration.dND.resize();
		assembler.addParameter(assembler.integration.dND);
	...
	void apply(int interval)
	{
		if (info::mesh->dimension == 2) {
			iterate_elements_gps<typename Assembler::NGP>(ElementJacobian2D(assembler.coords.node, assembler.integration.dN, assembler.integration.jacobiInversion, assembler.integration.jacobiDeterminant, assembler.integration.dND, interval));
		}
		if (info::mesh->dimension == 3) {
			iterate_elements_gps<typename Assembler::NGP>(ElementJacobian3D(assembler.coords.node, assembler.integration.dN, assembler.integration.jacobiInversion, assembler.integration.jacobiDeterminant, assembler.integration.dND, interval));
		}
	}
};
...
struct BoundaryIntegration: public BoundaryOperatorBuilder {
	...
	BoundaryIntegration(Assembler &assembler): BoundaryOperatorBuilder("BOUNDARY INTERGRATION"), assembler(assembler)
	{
	...
	bool build() override
	{
		for (size_t r = 0; r < info::mesh->boundaryRegions.size(); ++r) {
			assembler.integration.boundary.jacobian.regions[r].addInput(assembler.coords.boundary.node.regions[r]);
			assembler.integration.boundary.jacobian.regions[r].addInput(assembler.integration.boundary.dN.regions[r]);
			assembler.integration.boundary.jacobian.regions[r].isset = true;
			assembler.integration.boundary.jacobian.regions[r].resize();
			assembler.addParameter(assembler.integration.boundary.jacobian.regions[r]);
		}
		return true;
	}
	...
	void apply(int region, int interval)
	{
		if (info::mesh->boundaryRegions[region]->dimension == 2) {
			iterate_boundary_gps<typename Assembler::NGP>(BoundaryFaceJacobian(assembler.coords.boundary.node.regions[region], assembler.integration.boundary.dN.regions[region], assembler.integration.boundary.jacobian.regions[region], interval), region);
		}
		if (info::mesh->boundaryRegions[region]->dimension == 1) {
			if (info::mesh->dimension == 3) {
				iterate_boundary_gps<typename Assembler::NGP>(BoundaryEdge3DJacobian(assembler.coords.boundary.node.regions[region], assembler.integration.boundary.dN.regions[region], assembler.integration.boundary.jacobian.regions[region], interval), region);
			}
			if (info::mesh->dimension == 2) {
				iterate_boundary_gps<typename Assembler::NGP>(BoundaryEdge2DJacobian(assembler.coords.boundary.node.regions[region], assembler.integration.boundary.dN.regions[region], assembler.integration.boundary.jacobian.regions[region], interval), region);
			}
		}
	}
};
