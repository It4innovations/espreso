...
struct MatrixLargeDisplacementKernel<nodes, 2>: MatrixLargeDisplacement {
    ...
    void simd(Element &element, size_t gp)
    {
        ...
        for (size_t n = 0; n < nodes; ++n) {
            ...
            bt[1 * 2 * nodes + 2 * n] = element.dND[n * 2 + 1];
            bt[2 * 2 * nodes + 2 * n + 1] = element.dND[n * 2 + 0];
...
struct MatrixLargeDisplacementKernel<nodes, 3>: MatrixLargeDisplacement {
    ...
    void simd(Element &element, size_t gp)
    {
        ...
        for (size_t n = 0; n < nodes; n++) {
            for (int j = 0; j < 3; j++) {
                BL[0 * 3 * nodes + n + j * nodes] = F[j * 3 + 0] * element.dND[n * 3 + 0];
                BL[1 * 3 * nodes + n + j * nodes] = F[j * 3 + 1] * element.dND[n * 3 + 1];
                BL[2 * 3 * nodes + n + j * nodes] = F[j * 3 + 2] * element.dND[n * 3 + 2];
                BL[3 * 3 * nodes + n + j * nodes] = F[j * 3 + 0] * element.dND[n * 3 + 1] + F[j * 3 + 1] * element.dND[n * 3 + 0];
                BL[4 * 3 * nodes + n + j * nodes] = F[j * 3 + 1] * element.dND[n * 3 + 2] + F[j * 3 + 2] * element.dND[n * 3 + 1];
                BL[5 * 3 * nodes + n + j * nodes] = F[j * 3 + 0] * element.dND[n * 3 + 2] + F[j * 3 + 2] * element.dND[n * 3 + 0];
            }
        }
        ...
        for (size_t n = 0; n < nodes; ++n) {
            SIMD a = element.dND[n * 3 + 0] * sVec[0] + element.dND[n * 3 + 1] * sVec[3] + element.dND[n * 3 + 2] * sVec[5];
            SIMD b = element.dND[n * 3 + 0] * sVec[3] + element.dND[n * 3 + 1] * sVec[1] + element.dND[n * 3 + 2] * sVec[4];
            SIMD c = element.dND[n * 3 + 0] * sVec[5] + element.dND[n * 3 + 1] * sVec[4] + element.dND[n * 3 + 2] * sVec[2];

            SIMD nm = a * element.dND[n * 3 + 0] + b * element.dND[n * 3 + 1] + c * element.dND[n * 3 + 2];
            ...
            for (size_t m = n + 1; m < nodes; ++m) {
                SIMD nm = a * element.dND[m * 3 +0] + b * element.dND[m * 3 +1] + c * element.dND[m * 3 +2];
