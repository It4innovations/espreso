...
struct MatrixCorotationKernel<code, nodes, gps, 3>: MatrixCorotation {
    ...
    void simd(Element &element)
    {
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                for (size_t n = 0; n < nodes; ++n) {
                    F[i * 3 + j] = F[i * 3 + j] + element.displacement[n][j] * cdND[n][i];
                }
            }
        }
        F[0] = F[0] + load1(1.0);
        ...
        for (size_t n = 0; n < nodes; ++n) {
            ...
            ldisp[0] = ldisp[0] - lorigin[0];
            ldisp[1] = ldisp[1] - lorigin[1];
            ldisp[2] = ldisp[2] - lorigin[2];
            SIMD u_li[3]; multAtB<3, 3, 1>(u_lie, R, ldisp);
            u_lie[0 * nodes + n] = u_li[0];
            u_lie[1 * nodes + n] = u_li[1];
            u_lie[2 * nodes + n] = u_li[2];
        }
        ...
        SIMD fiep[3 * nodes]; multAB<3 * nodes, 3 * nodes, 1>(fiep, P_bar, f_ile);
        for (size_t n = 0; n < nodes; ++n) {
            SIMD fiepn[3]; fiepn[0] = fiep[3 * n + 0]; fiepn[1] = fiep[3 * n + 1]; fiepn[2] = fiep[3 * n + 2];
            SIMD fie[3]; multAB<3, 3, 1>(fie, R, fiepn);
            element.f[0 * nodes + n] = element.f[3 * n + 0] - fie[0];
            element.f[1 * nodes + n] = element.f[3 * n + 1] - fie[1];
            element.f[2 * nodes + n] = element.f[3 * n + 2] - fie[2];
        }

        SIMD Fn_bar[3 * nodes * 3];
        for (size_t n = 0; n < nodes; ++n) {
            Fn_bar[9 * n + 1] = -fiep[2 * nodes + n];
            Fn_bar[9 * n + 2] =  fiep[1 * nodes + n];
            Fn_bar[9 * n + 3] =  fiep[2 * nodes + n];
            Fn_bar[9 * n + 5] = -fiep[0 * nodes + n];
            Fn_bar[9 * n + 6] = -fiep[1 * nodes + n];
            Fn_bar[9 * n + 7] =  fiep[0 * nodes + n];
        }
