...
#include "regularization.harmonic.h"
...
template <typename T>
void RegularizationHarmonic<T>::set2D(esint domain)
{
...
template <typename T>
void RegularizationHarmonic<T>::set3D(esint domain)
{
	std::vector<esint> fixPoints;
	this->getFixPoints(fixPoints, domain);
...
template <typename T>
void RegularizationHarmonic<T>::setAnalytic()
{
...
template <typename T>
void RegularizationHarmonic<T>::updateAnalytic()
{
	#pragma omp parallel for
	for (size_t d = 0; d < feti.K.size(); ++d) {
...
template struct RegularizationHarmonic<double>;
