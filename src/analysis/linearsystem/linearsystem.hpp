...
void initSystem(LinearSystem<Assembler, Solver>* &system, Analysis *analysis)
{
	switch (analysis->configuration.solver) {
	case LoadStepSolverConfiguration::SOLVER::FETI:    system = new FETISystem<Analysis>(analysis); break;
	case LoadStepSolverConfiguration::SOLVER::HYPRE:   break;
	case LoadStepSolverConfiguration::SOLVER::MKLPDSS: system = new MKLPDSSSystem<Analysis>(analysis); break;
