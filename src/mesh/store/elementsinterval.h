...
struct ElementsInterval {
	esint begin, end;
	esint domain;
	int code, material, region;
	std::vector<int> regions; // in the (hopefully) rare case of intersected regions

	ElementsInterval(): begin(0), end(0), domain(-1), code(-1), material(-1), region(-1) {}
	ElementsInterval(esint begin, esint end): begin(begin), end(end), domain(-1), code(-1), material(-1), region(-1) {}
