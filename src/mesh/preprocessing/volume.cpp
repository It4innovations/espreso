...
void computeVolumeIndices(ElementStore *elements, const NodeStore *nodes)
{
	...
	auto bboxIndices = [&origin, &size, &grid, &step] (const Point &min, const Point &max, _Point<short> &imin, _Point<short> &imax) {
		...
		Point p = origin + Point(imin) * step;
		int inside[3] = { 0, 0, 0 };
	...
	auto cutConvex = [&] (const _Point<short> &imin, const _Point<short> &imax, const Point &v, const Point &n, std::vector<char> &isin) {
		Point p = origin + Point(imin) * step - v;
		int i = 0;
	...
	auto cutConcave = [&] (const _Point<short> &imin, const _Point<short> &imax, const Point &v0, const Point &v1, const Point &v2, std::vector<double> &angle) {
		Point p = origin + Point(imin) * step;
		int i = 0;
		for (short z = imin.z; z < imax.z; ++z, p.z += step) {
			for (short y = imin.y; y < imax.y; ++y, p.y += step) {
...
//		#pragma omp parallel for
		for (int t = 0; t < info::env::OMP_NUM_THREADS; ++t) {
			...
			for (auto e = elements->nodes->cbegin(t); e != elements->nodes->cend(t); ++e, ++epointers, ++eindex) {
				...
				if (bboxIndices(coomin, coomax, imin, imax)) {
					...
					case Element::CODE::TETRA4: {
						...
						for (auto triangle = (*epointers)->triangles->begin(); triangle != (*epointers)->triangles->end(); ++triangle) {
							const Point &v = coordinates[e->at(triangle->at(0))];
							const Point n = Point::cross(coordinates[e->at(triangle->at(2))] - v, coordinates[e->at(triangle->at(1))] - v);
					...
					case Element::CODE::POLYHEDRON: {
						...
						for (esint p = 0, pp = 1; p < e->front(); ++p, pp += e->at(pp) + 1) {
							...
							for (esint i = 0; i < e->at(pp); ++i) {
								const Point &v1 = coordinates[e->at(pp + 1 + i)];
								const Point &v2 = coordinates[e->at(pp + 1 + (i + 1) % e->at(pp))];
					...
					default:
						for (auto face = (*epointers)->faces->begin(); face != (*epointers)->faces->end(); ++face) {
							...
							pcenter /= face->size();
							for (size_t i = 0; i < face->size(); ++i) {
								const Point &v1 = coordinates[e->at(face->at(i))];
								const Point &v2 = coordinates[e->at(face->at((i + 1) % face->size()))];
								cutConcave(imin, imax, pcenter, v1, v2, angle);
							}
						}
						for (size_t i = 0; i < angle.size(); ++i) {
