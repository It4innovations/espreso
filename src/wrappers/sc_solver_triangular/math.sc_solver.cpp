...
#ifdef ESPRESO_USE_WRAPPER_SCSOLVER_TRIANGULAR
...
struct Schur_Complement_Solver_External_Representation
{
    ...
    trsm_csx_dny_tri<T,I> op_trsm_tri_L;
    herk_dnx_dny<T> op_herk_tri;
    I sc_size = 0;
    I other_size = 0;
    int stage = 0;
    struct config {
        trsm_csx_dny_tri<T,I>::config trsm_tri_cfg;
        herk_dnx_dny_tri<T>::config herk_tri_cfg;
        char order_X = '_';
        char order_L = '_';
    } cfg;
};
...
SchurComplementSolver<T,I>::SchurComplementSolver()
{
    ...
    // TODO: initialize config using getenv()
...
void SchurComplementSolver<T,I>::factorizeSymbolic()
{
    ...
    ext->A11_solver.get_permutation(ext->perm);
    ...
    if(ext->system_is_hermitian) {
        if(ext->A11_solver_factors_get_U) {
            ext->L_new = ext->U_new.make_transposed_reordered();
        ...
        if(ext->L_new.order != ext->config.order_L) {
            ext->L_new_reordered.set(ext->L_new.nrows, ext->L_new.ncols, ext->L_new.nnz, ext->config.order_L, AllocatorCPU_new::get_singleton());
...
void SchurComplementSolver<T,I>::factorizeNumericAndGetSc(Matrix_Dense<T,I,A> & sc, char uplo, T alpha)
{
    ...
    if(ext->system_is_hermitian) {
        ext->X.alloc();
        convert_csx_dny<T,I>::do_all(&ext->B_right, ext->X);
