...
void w_rocm_submatrix_ddnx_ddnx_noncontig<T,I>::internal_perform()
{
    ...
    if(map_primary == nullptr && map_secdary == nullptr) {
        CHECK(hipMemcpy2DAsync(d_M_dst->vals, d_M_dst->ld * sizeof(T), d_M_src->vals, d_M_src->ld * sizeof(T), d_M_src->get_size_secdary() * sizeof(T), d_M_src->get_size_primary(), hipMemcpyDefault, q->stream));
    }
    if(map_primary != nullptr && map_secdary == nullptr) {
...
template class w_rocm_submatrix_ddnx_ddnx_noncontig<T,I>;
