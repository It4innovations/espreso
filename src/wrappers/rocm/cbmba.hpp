...
// the memory pool is owned by the user, but used by the cbmba and should not be touched during the lifetime of the cbmba object

// if using this allocator in a loop like that, I think that schedule(dynamic,1) is very useful
// also, all allocations within one loop iteration should be in a critical region to avoid deadlocks, where all first allocations happen, which fill up the pool, and then no second allocation can occur because the pool is full
...
public:
    ...
    void * allocate(size_t num_bytes, size_t align = 1)
    {
        ...
        if(num_bytes > memory_pool_size) MY_ABORT("Not enough memory in the pool");
        if(memory_pool_size % align != 0) MY_ABORT("Align has to divide memory_pool_size");
        if(reinterpret_cast<std::uintptr_t>(memory_pool) % align != 0) MY_ABORT("Memory pool has to be aligned the same way as is the requested align");
        ...
        size_t new_block_end;
        {
            ...
            if(start_empty > transaction_end_fail) MY_ABORT("More memory requested in a single transaction then what is available in the pool");
...
private:
    ...
    void remove_from_full_blocks_vector(size_t mem_block_start_aligned)
    {
        ...
        if(it == end_iter) MY_ABORT("Bad remove from full blocks vector");
...
public:
    cbmba() = delete;
    cbmba(cbmba<T> const &) = default;
    cbmba(cbmba<T> &&) = default;
    ~cbmba() = default;
    cbmba<T> & operator=(cbmba<T> const &) = default;
    cbmba<T> & operator=(cbmba<T> &&) = default;
public:
    cbmba(cbmba_resource & res, size_t align_B_) : resource(res), align_B(align_B_) {}
    template<typename U> cbmba(cbmba<U> & other) : cbmba(other.resource, other.align_B) {}
    T * allocate(size_t count)
    {
        return reinterpret_cast<T*>(resource.allocate(count * sizeof(T), align_B));
    }
    ...
    template<typename U> friend class cbmba;
};
