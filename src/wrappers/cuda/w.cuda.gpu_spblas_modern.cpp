...
namespace spblas {
...
#else
        // if(stage == 'P') CHECK(cusparseSpSV_analysis    (h->h, _char_to_operation<T>(transpose), &one, matrix->d, rhs->d, sol->d, _sparse_data_type<T>(), CUSPARSE_SPSV_ALG_DEFAULT, descr_trsv->d, buffer_persistent)); // no need to do preprocess when update must happen anyway
        if(stage == 'U') CHECK(cusparseSpSV_analysis    (h->h, _char_to_operation<T>(transpose), &one, matrix->d, rhs->d, sol->d, _sparse_data_type<T>(), CUSPARSE_SPSV_ALG_DEFAULT, descr_trsv->d, buffer_persistent));
#endif
    ...
    void trsm(handle & h, char transpose_mat, char transpose_rhs, char transpose_sol, descr_matrix_csr & matrix, descr_matrix_dense & rhs, descr_matrix_dense & sol, descr_sparse_trsm & descr_trsm, buffer_sizes & buffersizes, void * buffer_persistent, void * buffer_tmp, char stage)
    {
        ...
        if(transpose_sol == 'N') {
...
#else
            // if(stage == 'P') CHECK(cusparseSpSM_analysis    (h->h, _char_to_operation<T>(transpose_mat), _char_to_operation<T>(transpose_rhs), &one, matrix->d, rhs->d, sol->d, _sparse_data_type<T>(), CUSPARSE_SPSM_ALG_DEFAULT, descr_trsm->d, buffer_persistent)); // it does not make sense to do Preprocessing. Update will be called anyway, which has to completely redo the analysis
            if(stage == 'U') CHECK(cusparseSpSM_analysis    (h->h, _char_to_operation<T>(transpose_mat), _char_to_operation<T>(transpose_rhs), &one, matrix->d, rhs->d, sol->d, _sparse_data_type<T>(), CUSPARSE_SPSM_ALG_DEFAULT, descr_trsm->d, buffer_persistent));
#endif
            ...
            trsm<T,I>(h, transpose_mat, transpose_rhs, 'N', matrix, rhs, sol, descr_trsm, buffersizes, buffer_persistent, buffer_tmp, stage);
        }
        else {
