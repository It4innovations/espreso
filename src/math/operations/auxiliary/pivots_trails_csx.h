...
public:
    ...
    void set_output_vector(VectorDenseView_new<size_t> * vec_);
    void perform();
    static void do_all(MatrixCsxView_new<T,I> * M, VectorDenseView_new<size_t> * vec, char row_col, char pivots_trails);
private:
    MatrixCsxView_new<T,I> * M = nullptr;
    VectorDenseView_new<size_t> * vec = nullptr;
    char row_col = '_';
    char pivots_trails = '_';
private:
    void perform_pivots(MatrixCsxView_new<T,I> & A, VectorDenseView_new<size_t> & pivots);
    void perform_trails(MatrixCsxView_new<T,I> & A, VectorDenseView_new<size_t> & trails);
};
