    ...
    if (data->dataType == NamedData::DataType::VECTOR || data->dataType == NamedData::DataType::TENSOR_SYMM) {
        ...
        for (size_t r = 1; r < info::mesh->elementsRegions.size(); ++r, ++part) {
            ...
            if (info::mesh->dimension == 3) {
                for (int d = 0; d < data->dimension; ++d) {
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(data->store[nindex * data->dimension + d]); });
                }
            }
            if (info::mesh->dimension == 2) {
                if (data->dataType == NamedData::DataType::VECTOR) {
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(data->store[nindex * data->dimension + 0]); });
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(data->store[nindex * data->dimension + 1]); });
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(0); });
                }
                if (data->dataType == NamedData::DataType::TENSOR_SYMM) {
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(data->store[nindex * data->dimension + 0]); });
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(data->store[nindex * data->dimension + 1]); });
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(0); });
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(data->store[nindex * data->dimension + 2]); });
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(0); });
                    niterator(region->nodeInfo.size, region->nodes->datatarray().data() + region->nodeInfo.nhalo, [&] (esint nindex) { _writer.float32(0); });
                }
            }
        ...
        for (size_t r = 1, part = 1; r < info::mesh->elementsRegions.size(); ++r, ++part) {
            ...
            for (int etype = 0; etype < static_cast<int>(Element::CODE::SIZE); etype++) {
                if (info::mesh->elementsRegions[r]->distribution.code[etype].totalSize) {
                    ...
                    if (info::mesh->dimension == 3) {
                        for (int d = 0; d < data->dimension; ++d) {
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(data->store[eindex * data->dimension + d]); });
                        }
                    }
                    if (info::mesh->dimension == 2) {
                        if (data->dataType == NamedData::DataType::VECTOR) {
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(data->store[eindex * data->dimension + 0]); });
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(data->store[eindex * data->dimension + 1]); });
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(0); });
                        }
                        if (data->dataType == NamedData::DataType::TENSOR_SYMM) {
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(data->store[eindex * data->dimension + 0]); });
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(data->store[eindex * data->dimension + 1]); });
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(0); });
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(data->store[eindex * data->dimension + 2]); });
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(0); });
                            eiterator(info::mesh->elementsRegions[r], etype, [&] (const ElementsInterval &interval, esint eindex) { _writer.float32(0); });
                        }
                    }
