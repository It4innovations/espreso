...
: Projector<T>(feti)
{
	this->e.resize();
	Gx.resize();
	iGGtGx.resize(this->feti->sinfo.R1size);

	_setG();
	if (this->feti->equalityConstraints->global) {
		_setDenseGGt();
	} else {
		_setSparseGGt();
	}
...
template<typename T>
void OrthogonalTFETISymmetric<T>::update()
{
	const typename FETI<T>::Regularization *R = this->feti->regularization;

	#pragma omp parallel for
	for (size_t d = 0; d < R->R1.domains.size(); ++d) {
		Vector_Dense<T> _e;
		_e.size = R->R1.domains[d].ncols;
		_e.vals = this->e.vals + Roffset[d] + Vector_Kernel<T>::offset;
		Matrix_Dense<T> _Rt;
		_Rt.nrows = R->R1.domains[d].ncols;
		_Rt.ncols = R->R1.domains[d].nrows;
		_Rt.vals = R->R1.domains[d].vals;
		math::apply(_e, T{1}, _Rt, T{0}, this->feti->f->domains[d]);
	}
	this->e.synchronize();
...
void OrthogonalTFETISymmetric<T>::_applyR(const Vector_Dense<T> &in, Vector_FETI<Vector_Dense, T> &out)
{
	...
	for (size_t d = 0; d < out.domains.size(); ++d) {
		Vector_Dense<T> y;
		y.size = this->feti->regularization->R1.domains[d].ncols;
		y.vals = in.vals + Roffset[d];

		math::apply(out.domains[d], T{1}, this->feti->regularization->R1.domains[d], T{0}, y);
	}
}
...
void OrthogonalTFETISymmetric<T>::_setG()
{
	...
	const Matrix_FETI<Matrix_CSR, T> *K = this->feti->K;
	const typename FETI<T>::Regularization *R = this->feti->regularization;
	...
	G.resize(nrows, this->feti->sinfo.lambdasLocal, 0);
	math::set(G.nrows + 1, G.rows, 1, 0);

	LMAPSize lsize(L->lmap.cend(), this->feti->sinfo.lambdasLocal);
	...
	G.resize(nrows, this->feti->sinfo.lambdasLocal, G.rows[G.nrows]);
...
void OrthogonalTFETISymmetric<T>::_setSparseGGt()
{
	...
	for (auto p = L->ordered.begin(); p != L->ordered.end(); ++p) {
		if (L->lmap[*p].from < K->decomposition->dbegin) { // halo
			kernels[L->lmap[*p].neigh].push_back(std::make_pair(R->R1.domains[L->lmap[*p].to - K->decomposition->dbegin].ncols, this->feti->sinfo.R1offset + Roffset[L->lmap[*p].to - K->decomposition->dbegin]));
	...
	GGt.resize(this->feti->sinfo.R1totalSize, this->feti->sinfo.R1totalSize, nnz);

	esint GGtOffsetIterator = GGtOffset;
	GGt.rows[0] = IDX;
	std::fill(noffset.begin(), noffset.end(), 0);
	for (size_t d = 0, i = 0, r = this->feti->sinfo.R1offset + 1; d < K->domains.size(); ++d) {
		while (i < L->ordered.size() && L->lmap[L->ordered[i]].from - K->decomposition->dbegin < (esint)d) { ++i; }
		for (esint k = 0; k < R->R1.domains[d].ncols; ++k, ++r) {
			size_t j = i;
			GGt.cols[GGtOffsetIterator++] = this->feti->sinfo.R1offset + Roffset[d] + IDX; // diagonal is always filled
			while (j < L->ordered.size() && L->lmap[L->ordered[j]].from - K->decomposition->dbegin == (esint)d) {
				...
				if (next->neigh == LMAP::LOCAL) {
					for (esint c = 0; c < R->R1.domains[next->to - K->decomposition->dbegin].ncols; ++c) {
						GGt.cols[GGtOffsetIterator++] = this->feti->sinfo.R1offset + Roffset[next->to - K->decomposition->dbegin] + c + IDX;
					}
	...
	if (!Communication::allGatherInplace(GGt.rows, this->feti->sinfo.R1offset + 1, G.nrows)) {
...
void OrthogonalTFETISymmetric<T>::_updateSparseGGt()
{
	...
	Matrix_Dense<T> eye;
	eye.resize(G.nrows, this->feti->sinfo.R1totalSize);
	math::set(eye, T{});
	for (esint r = 0; r < G.nrows; ++r) {
		eye.vals[r * this->feti->sinfo.R1totalSize + this->feti->sinfo.R1offset + r] = T{1};
	}
...
template<typename T>
void OrthogonalTFETISymmetric<T>::_print()
{
	if (info::ecf->output.print_matrices) {
		eslog::storedata(" STORE: feti/projector/{G, e, GGt, invGGt}\n");
		math::store(G, utils::filename(utils::debugDirectory(*this->feti->step) + "/feti/projector", "G").c_str());
		math::store(this->e, utils::filename(utils::debugDirectory(*this->feti->step) + "/feti/projector", "e").c_str());
		math::store(GGt, utils::filename(utils::debugDirectory(*this->feti->step) + "/feti/projector", "GGt").c_str());
		math::store(invGGt, utils::filename(utils::debugDirectory(*this->feti->step) + "/feti/projector", "invGGt").c_str());
	}
}
