...
template<typename T>
HFETIConjugateSymmetric<T>::HFETIConjugateSymmetric(FETI<T> &feti)
...
template<typename T>
HFETIConjugateSymmetric<T>::~HFETIConjugateSymmetric()
{

}

template<typename T>
void HFETIConjugateSymmetric<T>::set(const step::Step &step)
{

}

template<typename T>
void HFETIConjugateSymmetric<T>::update(const step::Step &step)
{
    ...
    if (reset) { // only different number of kernels
        ...
        for (auto v = vbegin; v != vend; ++v) {
            ...
            for (auto e = dual.clusters.edges[v->first].cbegin(); e != dual.clusters.edges[v->first].cend(); ++e) {
                if (dual.clusters.vertices.at(*e).rank != info::mpi::rank) {
...
template<typename T>
void HFETIConjugateSymmetric<T>::orthonormalizeKernels(const step::Step &step)
{
    std::vector<int> offset;
    Matrix_Dense<T> _R1;
    for (size_t d = 0; d < feti.KR1.size(); ++d) {
        offset.push_back(_R1.ncols);
        _R1.nrows = std::max(_R1.nrows, feti.KR1[d].nrows);
        _R1.ncols += feti.KR1[d].ncols;
    }
    _R1.resize(_R1.nrows, _R1.ncols);
    for (size_t d = 0; d < feti.KR1.size(); ++d) {
        for (int r = 0; r < feti.KR1[d].nrows; ++r) {
            math::blas::copy(feti.KR1[d].ncols, _R1.vals + _R1.ncols * r + offset[d], 1, feti.KR1[d].vals + feti.KR1[d].ncols * r, 1);
        }
    }
    math::orthonormalize(_R1);
    for (size_t d = 0; d < feti.KR1.size(); ++d) {
        for (int r = 0; r < feti.KR1[d].nrows; ++r) {
            math::blas::copy(feti.KR1[d].ncols, feti.KR1[d].vals + feti.KR1[d].ncols * r, 1, _R1.vals + _R1.ncols * r + offset[d], 1);
        }
    }
}

template<typename T>
void HFETIConjugateSymmetric<T>::_setG()
{
            ...
            for (size_t di = 0; di < v->second.offset.size(); ++di) {
                ...
                for (int c = 0; c < feti.B1[d].nrows; ++c) {
                    for (int i = feti.B1[d].rows[c]; i < feti.B1[d].rows[c + 1]; ++i) {
                        G.vals[G.rows[kr] + feti.D2C[d][c]] -= feti.KR1[d].vals[feti.KR1[d].ncols * kr + feti.B1[d].cols[i]] * feti.B1[d].vals[i];
                    }
                }
            }
        }
...
template<typename T>
void HFETIConjugateSymmetric<T>::_setGGt()
{
...
template<typename T>
void HFETIConjugateSymmetric<T>::_updateGGt()
{
    auto vbegin = dual.clusters.vertices.find(info::mpi::rank);
    auto vend   = dual.clusters.vertices.lower_bound(info::mpi::rank + 1);
    ...
    for (auto v = vbegin; v != vend; ++v) {
        for (int kr = 0; kr < v->second.kernel.size; ++kr, ++r1) {
            ...
            for (auto e = dual.clusters.edges[v->first].cbegin(); e != dual.clusters.edges[v->first].cend(); ++e) {
                for (int kc = 0; kc < dual.clusters.vertices[*e].kernel.size; ++kc) {
                    if (v->second.kernel.goffset + kr <= dual.clusters.vertices[*e].kernel.goffset + kc) {
                        while (nonzeros[r2] < dual.clusters.vertices[*e].kernel.goffset + kc) { ++r2; }
                        GGt.vals[c] = 0;
...
template struct HFETIConjugateSymmetric<double>;
template struct HFETIConjugateSymmetric<std::complex<double> >;
