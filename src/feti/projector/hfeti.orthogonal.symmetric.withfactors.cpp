...
template<typename T>
HFETIOrthogonalSymmetricWithFactors<T>::HFETIOrthogonalSymmetricWithFactors(FETI<T> &feti)
: Projector<T>(feti)
{
...
template<typename T>
HFETIOrthogonalSymmetricWithFactors<T>::~HFETIOrthogonalSymmetricWithFactors()
{

}

template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::info()
{
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::update(const step::Step &step)
{
    Vector_Dense<T> _e;
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::apply(const Vector_Dual<T> &x, Vector_Dual<T> &y)
{
    if (GGt.nrows) {
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::apply_e(const Vector_Kernel<T> &x, Vector_Dual<T> &y)
{
    if (GGt.nrows) {
        math::set(y, T{0});
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::apply_R(const Vector_Kernel<T> &x,  std::vector<Vector_Dense<T> > &y)
{
    if (GGt.nrows) {
        _applyR(x, y);
    } else {
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::apply_Ra(const Vector_Dual<T> &x,  std::vector<Vector_Dense<T> > &y)
{
    if (GGt.nrows) {
        _applyG(x, Gx);
        _applyInvGGt(Gx, iGGtGx);
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::apply_invU(const Vector_Kernel<T> &x, Vector_Kernel<T> &y)
{
    if (GGt.nrows) {
        math::set(y, T{0});
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::apply_invL(const Vector_Kernel<T> &x, Vector_Kernel<T> &y)
{
    if (GGt.nrows) {
        math::set(y, T{0});
        Vector_Dense<T> local;
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::apply_GtinvU(const Vector_Kernel<T> &x, Vector_Dual<T> &y)
{
    if (GGt.nrows) {
        math::set(y, T{0});
        _applyInvU(x, iGGtGx);
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::apply_invLG(const Vector_Dual<T> &x, Vector_Kernel<T> &y)
{
    if (GGt.nrows) {
        _applyG(x, Gx);
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_applyG(const Vector_Dual<T> &in, Vector_Kernel<T> &out)
{
    #pragma omp parallel for
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_applyInvGGt(const Vector_Kernel<T> &in, Vector_Dense<T> &out)
{
    Vector_Kernel<T> mid;
//    _applyInvL(in, mid);
    #pragma omp parallel for
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_applyInvL(const Vector_Kernel<T> &in, Vector_Dense<T> &out)
{
    #pragma omp parallel for
    for (int t = 0; t < info::env::threads; ++t) {
        Matrix_Dense<T> a;
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_applyInvU(const Vector_Kernel<T> &in, Vector_Dense<T> &out)
{
    #pragma omp parallel for
    for (int t = 0; t < info::env::threads; ++t) {
        Matrix_Dense<T> a;
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_applyGt(const Vector_Dense<T> &in, const T &alpha, Vector_Dual<T> &out)
{
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_applyR(const Vector_Dense<T> &in, std::vector<Vector_Dense<T> > &out)
{
    Vector_Dense<T> y;
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_applyR(const Vector_Kernel<T> &in, std::vector<Vector_Dense<T> > &out)
{
    Vector_Dense<T> y;
    y.size = cinfo[0].kernels;
    y.vals = in.vals + cinfo[0].koffset;
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_computeDualGraph()
{
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_setG()
{
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_updateG()
{
    math::set(G, T{0});
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_setGGt()
{
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_updateGGt()
{
    const int IDX = Indexing::CSR;
    for (int kr = 0; kr < feti.sinfo.R1size; ++kr) {
...
template<typename T>
void HFETIOrthogonalSymmetricWithFactors<T>::_print(const step::Step &step)
{
...
template struct HFETIOrthogonalSymmetricWithFactors<double>;
template struct HFETIOrthogonalSymmetricWithFactors<std::complex<double> >;
