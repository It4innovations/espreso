...
#include "../../../old_physics/transient/elasticity/assembler.h"
...
void TransientElasticity::saveMeshProperties(output::ResultStore &store)
{

}

void TransientElasticity::saveMeshResults(output::ResultStore &store, const std::vector<std::vector<double> > &results)
{
...
static void processElement(DenseMatrix &Ke, DenseMatrix &Me, std::vector<double> &fe, const espreso::Mesh &mesh, size_t subdomain, const Element* element)
{
	...
	for (size_t gp = 0; gp < element->gaussePoints(); gp++) {
		...
		Me.multiply(N[gp], N[gp], material->get(MATERIAL_PARAMETER::DENSITY)->evaluate(0) * detJ * weighFactor[gp] * CP, 1, true);
...
void TransientElasticity::assembleStiffnessMatrix(const Element* e, DenseMatrix &Ke, std::vector<double> &fe, std::vector<eslocal> &dofs) const
{
	ESINFO(GLOBAL_ERROR) << "Implement assembleStiffnessMatrix";
}

void TransientElasticity::makeStiffnessMatricesRegular()
{
...
void TransientElasticity::composeSubdomain(size_t subdomain)
{
	...
	for (eslocal i = partition[subdomain]; i < partition[subdomain + 1]; i++) {
		...
		for (size_t i = 0; i < pointDOFs.size() * element->nodes(); i++) {
			size_t row = pointDOFs.size() * (element->node(i % element->nodes())) + i / element->nodes();
			for (size_t j = 0; j < pointDOFs.size() * element->nodes(); j++) {
				size_t column = pointDOFs.size() * (element->node(j % element->nodes())) + j / element->nodes();
		...
		for (size_t r = 0; r < element->nodes(); r++) {
			for (size_t c = 0; c < element->nodes(); c++) {
				for (size_t k = 0; k < pointDOFs.size(); k++) {
					_M(pointDOFs.size() * element->node(r) + k, pointDOFs.size() * element->node(c) + k) = Me(r, c);
				}
			}
		}
	}
