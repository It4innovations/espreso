...
//#include "../../configuration/physics/shallowwatter2d.h"
#include "shallowwatter2d.h"
...
ShallowWatter2D::ShallowWatter2D(Mesh *mesh, Instance *instance) //, const AdvectionDiffusion2DConfiguration &configuration)
: Physics2D("ADVECTION DIFFUSION 2D", mesh, instance)//, _configuration(configuration)
{

}

MatrixType ShallowWatter2D::getMatrixType(const Step &step, size_t domain) const
{
...
void ShallowWatter2D::prepareTotalFETI()
{
...
void ShallowWatter2D::analyticRegularization(size_t domain)
{
...
void ShallowWatter2D::assembleMaterialMatrix(const Step &step, const Element *e, eslocal node, double temp, DenseMatrix &K) const
{
...
void ShallowWatter2D::processElement(const Step &step, Matrices matrices, const Element *e, DenseMatrix &Ke, DenseMatrix &Me, DenseMatrix &Re, DenseMatrix &fe, const std::vector<Solution*> &solution) const
{
	bool CAU = 1;//_configuration.stabilization == AdvectionDiffusion2DConfiguration::STABILIZATION::CAU;
	...
	for (size_t gp = 0; gp < e->gaussePoints(); gp++) {
		...
		Ce(0, 0) += 0; //configuration.sigma * h_e * norm_u_e;
		Ce(1, 1) += 0; //configuration.sigma * h_e * norm_u_e;
...
void ShallowWatter2D::processFace(const Step &step, Matrices matrices, const Element *e, DenseMatrix &Ke, DenseMatrix &Me, DenseMatrix &Re, DenseMatrix &fe, const std::vector<Solution*> &solution) const
{
	ESINFO(ERROR) << "Advection diffusion 2D cannot process face";
}

void ShallowWatter2D::processEdge(const Step &step, Matrices matrices, const Element *e, DenseMatrix &Ke, DenseMatrix &Me, DenseMatrix &Re, DenseMatrix &fe, const std::vector<Solution*> &solution) const
{
...
void ShallowWatter2D::processNode(const Step &step, Matrices matrices, const Element *e, DenseMatrix &Ke, DenseMatrix &Me, DenseMatrix &Re, DenseMatrix &fe, const std::vector<Solution*> &solution) const
{

}

void ShallowWatter2D::processSolution(const Step &step)
{
