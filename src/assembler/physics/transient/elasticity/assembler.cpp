...
void TransientElasticity::composeSubdomain(size_t subdomain)
{
	...
	for (eslocal i = partition[subdomain]; i < partition[subdomain + 1]; i++) {
		...
		for (size_t i = 0; i < DOFs * element->size(); i++) {
			size_t row = DOFs * (element->node(i % element->size())) + i / element->size();
			for (size_t j = 0; j < DOFs * element->size(); j++) {
				size_t column = DOFs * (element->node(j % element->size())) + j / element->size();
		...
		for (size_t r = 0; r < element->size(); r++) {
			for (size_t c = 0; c < element->size(); c++) {
				for (size_t k = 0; k < DOFs; k++) {
					_M(DOFs * element->node(r) + k, DOFs * element->node(c) + k) = Me(r, c);
				}
			}
		}
	}
