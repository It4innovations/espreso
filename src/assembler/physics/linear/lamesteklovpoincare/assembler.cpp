...
void LameSteklovPoincare::saveMeshProperties(store::Store &store)
{

}

void LameSteklovPoincare::saveMeshResults(store::Store &store, const std::vector<std::vector<double> > &results)
{
...
void LameSteklovPoincare::assembleStiffnessMatrix(const Element* e, DenseMatrix &Ke, std::vector<double> &fe, std::vector<eslocal> &dofs) const
{
...
void LameSteklovPoincare::composeSubdomain(size_t subdomain)
{
	...
	for (eslocal i = partition[subdomain], index = 0; i < partition[subdomain + 1]; i++, index++) {
