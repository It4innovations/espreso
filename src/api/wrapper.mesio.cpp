...
struct MESIOData {
	MESIOData()
	{
		etypes.reserve(espreso::info::mesh->elements->size);
...
void MESIOElements(
	...
	MESIOInt**      enodesData)
{
	*offset = info::mesh->elements->offset;
...
void MESIOElementsCounters(
	...
	MESIOInt*       totalSize)
{
	*offset = info::mesh->elementsRegions.front()->eoffsets[etype];
	*totalSize = info::mesh->elementsRegions.front()->ecounters[etype];
}
...
void MESIOElementsRegionCounters(
	...
	MESIOInt*       totalSize)
{
	*offset = info::mesh->elementsRegions[region]->eoffsets[etype];
	*totalSize = info::mesh->elementsRegions[region]->ecounters[etype];
}
	...
	MESIOInt**      elementData)
{
	...
	*dimension = info::mesh->boundaryRegions[region]->dimension;
	if (*dimension) {
		*size = info::mesh->boundaryRegions[region]->size;
...
void MESIOBoundaryRegionCounters(
	...
	MESIOInt*       totalSize)
{
	*offset = info::mesh->boundaryRegions[region]->eoffsets[etype];
	*totalSize = info::mesh->boundaryRegions[region]->ecounters[etype];
}
