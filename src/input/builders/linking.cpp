...
void linkup(const TemporalMesh<MergedNodes, ClusteredElements> &merged, const TemporalMesh<LinkedNodes, ClusteredElements> &linked)
{
	...
	std::vector<esint> rankDistribution(merged.nodes->offsets.size() + 1), duplicationIndex(merged.nodes->offsets.size(), -1);
	for (size_t i = 0, j = 0; i < required.size(); ++i) { // required size is proportional to clustered.noffsets
		while (j < merged.nodes->offsets.size() && merged.nodes->offsets[j] < required[i]) { ++j; }
		if (j < merged.nodes->offsets.size() && merged.nodes->offsets[j] == required[i]) {
			++rankDistribution[j];
		}
	}
	...
	for (size_t i = 0; i < merged.nodes->duplication.size(); ++i) {
		...
		rankDistribution[merged.nodes->duplication[i].origin] += rankDistribution[merged.nodes->duplication[i].duplicate];
		duplicationIndex[merged.nodes->duplication[i].origin] = i;      // always last occurrence
		duplicationIndex[merged.nodes->duplication[i].duplicate] = i; // always pointer to offset
	}
	for (size_t i = 0; i < merged.nodes->duplication.size(); ++i) {
		rankDistribution[merged.nodes->duplication[i].duplicate] = rankDistribution[merged.nodes->duplication[i].origin];
	}
	...
	{ // build nodes
		...
		linked.nodes->offsets.reserve(required.size());
		linked.nodes->coordinates.reserve(required.size());
		linked.nodes->rankDistribution.reserve(required.size() + 1);
		...
		for (auto dup = merged.nodes->duplication.begin(); dup != merged.nodes->duplication.end(); ++dup) {
			linked.nodes->duplication.push_back({ merged.nodes->offsets[dup->origin], merged.nodes->offsets[dup->duplicate] }); // it can include unnecessary nodes
		}

		auto push = [&] (const esint &offset, const _Point<esfloat> *coo, const esint &nranks, const int *ranks) {
			linked.nodes->offsets.push_back(offset);
			linked.nodes->coordinates.push_back(*coo);
			for (esint n = 0; n < nranks; ++n) {
				linked.nodes->rankData.push_back(ranks[n]);
				if (linked.nodes->rankData.back() != info::mpi::rank) {
					neighbors.insert(linked.nodes->rankData.back());
				}
			}
		...
		for (size_t i = 0, j = 0; i < required.size(); ++i) {
			while (projection.size() && projection.top().offset <= required[i]) { // it works since duplicate < origin
				while (projection.size() && projection.top().offset <= required[i]) {
					if (projection.top().offset < required[i]) {
						...
						while (projection.size() && linked.nodes->offsets.back() == projection.top().offset) { projection.pop(); }
					} else {
			...
			while (j < merged.nodes->offsets.size() && merged.nodes->offsets[j] < required[i]) { ++j; }
			if (j < merged.nodes->offsets.size() && merged.nodes->offsets[j] == required[i]) { // local node
				if (duplicationIndex[j] != -1 && merged.nodes->offsets[j] == linked.nodes->duplication[duplicationIndex[j]].duplicate) {
					projection.push({
						linked.nodes->duplication[duplicationIndex[j]].origin,
						rankDistribution[j + 1] - rankDistribution[j],
						rankData.data() + rankDistribution[j],
						&merged.nodes->coordinates[j]});
				} else {
					push(merged.nodes->offsets[j], &merged.nodes->coordinates[j], rankDistribution[j + 1] - rankDistribution[j], rankData.data() + rankDistribution[j]);
				}
			} else { // adjacent node
				for (size_t r = 0; r < recvOffset.size(); ++r) {
					if (recvOffset[r] < received[r].front() && required[i] == received[r][recvOffset[r] + 1]) {
						...
						for (esint n = 0; n < nduplications; ++n) {
							linked.nodes->duplication.push_back({ ranks[r][rankOffset[r] + nranks + 2 * n], ranks[r][rankOffset[r] + nranks + 2 * n + 1] });
							if (linked.nodes->duplication.back().duplicate == required[i]) {
								project = linked.nodes->duplication.back().origin;
							}
						}
						if (project != -1) {
		...
		for (auto it = linked.nodes->duplication.begin(); it != linked.nodes->duplication.end(); ++it) {
			linked.nodes->g2l[it->duplicate] = linked.nodes->g2l[it->origin];
		}

		linked.nodes->neighbors.clear();
		for (auto n = neighbors.begin(); n != neighbors.end(); ++n) {
			linked.nodes->neighbors.push_back(*n);
		}
		std::sort(linked.nodes->neighbors.begin(), linked.nodes->neighbors.end());
	}
...
static void exchangeUnknown(const TemporalMesh<MergedNodes, ClusteredElements> &merged, const TemporalMesh<LinkedNodes, ClusteredElements> &linked, std::vector<esint> &adjacent, std::vector<std::vector<esint> > &send, std::vector<std::vector<esint> > &recv)
{
	auto permute = [&] () {
		...
		for (size_t p = 0; p < permutation.size(); ++p) {
			...
			_neighbors[p] = linked.nodes->neighbors[permutation[p]];
		}
		send.swap(_send);
		recv.swap(_recv);
	...
	for (size_t i = 0; i < unknown.size(); ++i) {
		...
		if (it != merged.nodes->offsets.end() && *it == unknown[i].offset) {
			size_t n = it - merged.nodes->offsets.begin();
	...
	if (found.size()) { // insert surely found nodes
		...
		auto end = begin;
		auto rank = linked.nodes->neighbors.begin();
		auto rankEnd = linked.nodes->neighbors.end();
		auto ranks = linked.nodes->neighbors.size();
		while (begin != found.end() && begin->rank == info::mpi::rank) {
			...
			if (rank != rankEnd && *rank == begin->holder) {
				_recv = &recv[rank - linked.nodes->neighbors.begin()];
				_recv->front() = (esint)(end - begin);
			} else {
				linked.nodes->neighbors.push_back(begin->holder);
				rank = linked.nodes->neighbors.begin();
				rankEnd = rank + ranks;
				send.push_back({ 0 });
