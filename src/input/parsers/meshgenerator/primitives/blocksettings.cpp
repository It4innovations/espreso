...
BlockSettings::BlockSettings(const BlockGeneratorConfiguration &configuration)
{
    ...
    start = Triple<long>(configuration.start_x.evaluator->evaluate() / MeshGenerator::precision, configuration.start_y.evaluator->evaluate() / MeshGenerator::precision, configuration.start_z.evaluator->evaluate() / MeshGenerator::precision);
    end   = Triple<long>(
            (long)std::round((configuration.start_x.evaluator->evaluate() + configuration.length_x.evaluator->evaluate()) / (.1 * MeshGenerator::precision)) / 10,
            (long)std::round((configuration.start_y.evaluator->evaluate() + configuration.length_y.evaluator->evaluate()) / (.1 * MeshGenerator::precision)) / 10,
            (long)std::round((configuration.start_z.evaluator->evaluate() + configuration.length_z.evaluator->evaluate()) / (.1 * MeshGenerator::precision)) / 10);
