...
summary_file = basedir + "/summary/" + summary_dir_timestamp + "/summary_phase3.csv"
                ...
                for trsm_strategy in trsm_strategy_list:
                    ...
                    for all_ndofs_unique_idx in range(len(all_ndofs_unique)):
                        ...
                        for sign_idx in range(2):
                            sign = (-1) ** sign_idx
                            csv_data_07 = [row for row in csv_data_06 if (int(row[col_trsm_partition_parameter]) * sign > 0)]
                            if(len(csv_data_07) == 0):
                                continue

                            csv_data_08 = sorted(csv_data_07, key=lambda row: int(row[col_trsm_partition_parameter]))
                            vals_x_str = [row[col_trsm_partition_parameter] for row in csv_data_08]
