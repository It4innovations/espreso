...
summary_file = basedir + "/summary/" + summary_dir_timestamp + "/summary_phase4.csv"
    ...
    for physics in physics_list:
        ...
        for dimension in dimensions_list:
            ...
            for element_type in element_types_list:
                ...
                for herk_strategy in herk_strategy_list:
                    csv_data_05 = [row for row in csv_data_04 if (row[col_herk_strategy] == herk_strategy)]
                    ...
                    imgname = dualop_cpugpu + "-" + physics + "-" + dimension + "D-" + element_type + "-" + herk_strategy
                    ...
                    for all_ndofs_unique_idx in range(len(all_ndofs_unique)):
                        ...
                        csv_data_06 = [row for row in csv_data_05 if (int(row[col_n_dofs]) == ndofs)]

                        for sign_idx in range(2):
                            sign = (-1) ** sign_idx
                            csv_data_07 = [row for row in csv_data_06 if (int(row[col_herk_partition_parameter]) * sign > 0)]
                            if(len(csv_data_07) == 0):
                                continue

                            csv_data_08 = sorted(csv_data_07, key=lambda row: int(row[col_herk_partition_parameter]))
                            vals_x_str = [row[col_herk_partition_parameter] for row in csv_data_08]
                            vals_y_str = [row[col_time_per_subdomain] for row in csv_data_08]
