...
graphs_dir_png = graphs_dir + "/png_times_separate_1"
graphs_dir_tikz = graphs_dir + "/tikz_times_separate_1"
...
for physics in physics_list:
    for element_type in element_types_list:
        for approach in approach_list:
            ...
            grafs_nrows = len(part_labels) * len(division_variant_cols)
            ...
            for n_dofs_cluster_idx in range(len(n_dofs_cluster_list)):
                ...
                for division_variant_idx in range(len(division_variant_cols)):
                    ...
                    for part_idx in range(len(part_labels)):
                        ...
                        graph_row = 3 * division_variant_idx + part_idx
                        graph_col = n_dofs_cluster_idx

                        title = part_label + ", " + n_dofs_cluster + " DOFs in cluster"
            ...
            for graph_row in range(grafs_nrows):
                for graph_col in range(grafs_ncols):
                    ...
                    a.set_xlabel(division_variant_labels[graph_row // 3])
                    a.set_ylabel("Time [ms]")
                    tp.xlabel = division_variant_labels[graph_row // 3]
                    tp.ylabel = "Time [ms]"
            ...
            for graph_row in range(grafs_nrows):
                for graph_col in range(grafs_ncols):
                    path = graphs_dir_tikz + "/" + imgname + "-" + part_labels[graph_row % 3] + "-" + division_variant_labels_lite[graph_row // 3] + "-" + n_dofs_cluster_list[graph_col] + ".tex"
