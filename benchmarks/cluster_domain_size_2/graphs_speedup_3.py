...
graphs_dir_png = graphs_dir + "/png_speedup_3"
graphs_dir_tikz = graphs_dir + "/tikz_speedup_3"
...
niters_list = [1,3,10,30,100,300,1000,3000,10000, 0,float("inf")]
            ...
            for n_dofs_cluster_idx in range(len(n_dofs_cluster_list)):
                ...
                for feti_method_idx in range(len(feti_method_list)):
                    ...
                    for part_idx in range(len(part_labels)):
                        ...
                        times_ic_update_str = [row[part_phase_cols[part_idx][0]] for row in csv_data_06 if (row[col_approach] == "IMPLICIT_CPU")]
                        times_eg_update_str = [row[part_phase_cols[part_idx][0]] for row in csv_data_06 if (row[col_approach] == "EXPLICIT_GPU")]
                        times_ic_apply_str = [row[part_phase_cols[part_idx][1]] for row in csv_data_06 if (row[col_approach] == "IMPLICIT_CPU")]
                        times_eg_apply_str = [row[part_phase_cols[part_idx][1]] for row in csv_data_06 if (row[col_approach] == "EXPLICIT_GPU")]
                        times_ic_update = [float(val) if len(val) > 0 else float("nan") for val in times_ic_update_str]
                        times_eg_update = [float(val) if len(val) > 0 else float("nan") for val in times_eg_update_str]
                        times_ic_apply = [float(val) if len(val) > 0 else float("nan") for val in times_ic_apply_str]
                        times_eg_apply = [float(val) if len(val) > 0 else float("nan") for val in times_eg_apply_str]
                        ...
                        for niters_idx in range(len(niters_list)):
                            ...
                            if niters == float("inf"):
                                for i in range(len(vals_x)):
                                    time_ic = times_ic_apply[i]
                                    time_eg = times_eg_apply[i]
                                    vals_y[i] = time_ic / time_eg
                            else:
                                for i in range(len(vals_x)):
                                    time_ic = times_ic_update[i] + niters * times_ic_apply[i]
                                    time_eg = times_eg_update[i] + niters * times_eg_apply[i]
                                    vals_y[i] = time_ic / time_eg

                            label = str(niters) + " applications"
                            color = ["red", "cyan", "green", "magenta", "blue", "grey"][niters_idx // 2]
                            linestyle = ["-", "--"][niters_idx % 2]
                            if niters == 0:
                                color = "black"
                                linestyle = "--"
                                label = "update only"
