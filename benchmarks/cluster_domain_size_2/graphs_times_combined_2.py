...
graphs_dir_png = graphs_dir + "/png_times_combined_2"
graphs_dir_tikz = graphs_dir + "/tikz_times_combined_2"
...
niters_list = [1,10,100,1000,10000]
        ...
        for division_variant_idx in range(len(division_variant_cols)):
            ...
            for n_dofs_cluster_idx in range(len(n_dofs_cluster_list)):
                ...
                for feti_method_idx in range(len(feti_method_list)):
                    ...
                    for part_idx in range(len(part_labels)):
                        ...
                        vals_x_str = list(set([row[division_variant_col] for row in csv_data_06]))
                        vals_x = sorted([float(x) for x in vals_x_str])

                        for approach_idx in range(len(approach_list)):
                            approach = approach_list[approach_idx]
                            if approach == "EXPLICIT_CPU": continue
                            csv_data_07 = [row for row in csv_data_06 if (row[col_approach] == approach)]

                            times_update_str = [row[part_phase_cols[part_idx][0]] for row in csv_data_07]
                            times_apply_str = [row[part_phase_cols[part_idx][1]] for row in csv_data_07]
                            times_update = [(float("nan") if y == "" else float(y)) for y in times_update_str]
                            times_apply = [(float("nan") if y == "" else float(y)) for y in times_apply_str]
                            ...
                            for niters_idx in range(len(niters_list)):
                                ...
                                for i in range(len(vals_x)):
                                    vals_y[i] = times_update[i] + niters * times_apply[i]

                                color = ["red", "cyan", "green", "magenta", "blue"][niters_idx]
                                linestyle = ["--", "-.", ":"][approach_idx]
                                label = approach + ", " + str(niters) + " iterations"
            ...
            for graph_row in range(grafs_nrows):
                for graph_col in range(grafs_ncols):
                    ...
                    a.set_xlabel(division_variant_label)
                    a.set_ylabel("Time [ms]")
                    tp.xlabel = division_variant_label
                    tp.ylabel = "Time [ms]"
