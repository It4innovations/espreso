...
graphs_dir_png = graphs_dir + "/png_times_combined_1"
graphs_dir_tikz = graphs_dir + "/tikz_times_combined_1"
    ...
    for element_type in element_types_list:
        for approach in approach_list:
            for division_variant_idx in range(len(division_variant_cols)):
                ...
                csv_data_02 = [row for row in csv_data_00 if ((row[col_physics] == physics) and (row[col_element_type] == element_type) and (row[col_approach] == approach))]
                ...
                imgname = physics + "-" + dimension + "D-" + element_type + "-" + approach + "-" + division_variant_label_lite
                ...
                for n_dofs_cluster_idx in range(len(n_dofs_cluster_list)):
                    ...
                    for feti_method_idx in range(len(feti_method_list)):
                        ...
                        for part_idx in range(len(part_labels)):
                            ...
                            vals_x_str = [row[division_variant_col] for row in csv_data_06]
                            vals_x = [float(x) for x in vals_x_str]

                            times_update_str = [row[part_phase_cols[part_idx][0]] for row in csv_data_06]
                            times_apply_str = [row[part_phase_cols[part_idx][1]] for row in csv_data_06]
