...
header_2 = ["feti_method", "approach", "dual_operator", "preconditioner", "projector", "element_type", "domains_x", "domains_y", "domains_z", "domains_per_cluster", "elements_x", "elements_y", "elements_z", "elements_per_domain", "n_dofs_domain_calc", "n_dofs_domain_read", "n_dofs_interface_read", "n_dofs_total_calc", "n_dofs_total_read"]
header_4 = ["exitcode", "errfile", "timeoutfile"]
header_5 = ["dualop_update_time", "precond_update_time", "dualop_apply_time", "precond_apply_time", "total_update_time", "total_apply_time"]
...
for task_dir_name in os.listdir(tasks_dir):
    ...
    for field in header_2:
        if field == "":
            ...
            line = [line for line in config_file_lines if (field.upper() in line)][0]
            value = line.split("=")[1]
    ...
    measured_lines_finishd = [l for l in out_file_lines if ("finishd" in l)]
    measured_lines_update = [l for l in measured_lines_finishd if ("::update" in l)]
    measured_lines_apply = [l for l in measured_lines_finishd if ("::apply" in l)]
    measured_lines_update_dualop  = [l for l in measured_lines_update if (("HybridFETI" in l) or ("TotalFETI" in l))]
    measured_lines_update_precond = [l for l in measured_lines_update if ("Dirichlet" in l)]
    measured_lines_apply_dualop  = [l for l in measured_lines_apply if (("::apply (vector)" in l) and (("HybridFETI" in l) or ("TotalFETI" in l)))]
    measured_lines_apply_precond = [l for l in measured_lines_apply if ("Dirichlet" in l)]

    total_update_time = 0.0
    total_apply_time = 0.0

    if len(measured_lines_update_dualop) > 0:
        measured_line = measured_lines_update_dualop[-1]
        measured_time = measured_line[100:-3].replace(" ", "")
        summstring.write(measured_time + ";")
        total_update_time += float(measured_time)
    else:
        summstring.write(";")
        total_update_time = float("nan")

    if len(measured_lines_update_precond) > 0:
        measured_line = measured_lines_update_precond[-1]
        measured_time = measured_line[100:-3].replace(" ", "")
        summstring.write(measured_time + ";")
        total_update_time += float(measured_time)
    else:
    ...
    if len(measured_lines_apply_dualop) > 0:
        if len(measured_lines_apply_dualop) == 1:
            measured_lines_apply_to_use = measured_lines_apply_dualop
        elif len(measured_lines_apply_dualop) <= 3:
            measured_lines_apply_to_use = measured_lines_apply_dualop[1:]
        else:
            measured_lines_apply_to_use = measured_lines_apply_dualop[2:]
        vals = [float(l[100:-3].replace(" ", "")) for l in measured_lines_apply_to_use]
        avg = sum(vals) / len(vals)
        summstring.write(str(avg) + ";")
        total_apply_time += avg
    else:
        summstring.write(";")
        total_apply_time = float("nan")

    if len(measured_lines_apply_precond) > 0:
        if len(measured_lines_apply_precond) == 1:
            measured_lines_apply_to_use = measured_lines_apply_precond
        elif len(measured_lines_apply_precond) <= 3:
            measured_lines_apply_to_use = measured_lines_apply_precond[1:]
        else:
            measured_lines_apply_to_use = measured_lines_apply_precond[2:]
        vals = [float(l[100:-3].replace(" ", "")) for l in measured_lines_apply_to_use]
        avg = sum(vals) / len(vals)
        summstring.write(str(avg) + ";")
        total_apply_time += avg
    ...
    if total_update_time == total_update_time: summstring.write(str(total_update_time))
    summstring.write(";")
    if total_apply_time == total_apply_time: summstring.write(str(total_apply_time))
    summstring.write(";")
...
print("  ./benchmarks/cluster_domain_size_2/graphs.sh " + summfile)
