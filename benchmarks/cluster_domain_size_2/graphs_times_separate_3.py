...
graphs_dir_png = graphs_dir + "/png_times_separate_3"
graphs_dir_tikz = graphs_dir + "/tikz_times_separate_3"
...
for physics in physics_list:
    for element_type in element_types_list:
        for division_variant_idx in range(len(division_variant_cols)):
            ...
            grafs_nrows = len(approach_list) * len(feti_method_list)
                    ...
                    for part_idx in range(len(part_labels)):
                        ...
                        for approach_idx in range(len(approach_list)):
                            ...
                            for phase_idx in range(len(phase_labels)):
                                ...
                                vals_x_str = [row[division_variant_col] for row in csv_data_07]
                                vals_y_str = [row[value_col] for row in csv_data_07]
                                vals_x = [float(x) for x in vals_x_str]
                                vals_y = [(float("nan") if y == "" else float(y)) for y in vals_y_str]

                                color = ["red", "green", "blue"][approach_idx]
                                linestyle = "-" if phase_idx == 0 else "--"
                                label = approach + " " + phase_label
