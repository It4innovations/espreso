...
x_axis_variant_labels = ["Optimal number of subdomains per cluster", "Optimal number of DOFs per subdomain"]
...
for physics in physics_list:
    for element_type in element_types_list:
        for dualop in dual_operator_list:
            ...
            graph_nrows = len(x_axis_variant_cols)
            graph_ncols = len(feti_method_list) * len(update_apply_variant_cols)
            ...
            for feti_method_idx in range(len(feti_method_list)):
                ...
                for x_variant_idx in range(len(x_axis_variant_cols)):
                    ...
                    for update_apply_idx in range(len(update_apply_variant_cols)):
                        ...
                        graph_col = 2 * feti_method_idx + update_apply_idx
                        ...
                        for n_dofs_cluster_idx in range(len(n_dofs_cluster_list)):
                            ...
                            times_str = [row[update_apply_col] for row in csv_data_06]
                            times = [(float("nan") if len(x) == 0 else float(x)) for x in times_str]
                            division_variants_str = [row[x_variant_col] for row in csv_data_06]
                        ...
                        color = "blue" if update_apply_idx == 0 else "red"
                        linestyle = "-"
                        ...
                        axs[graph_row,graph_col].set_title(feti_method + "-" + update_apply_name)
            ...
            for graph_row in range(graph_nrows):
                for graph_col in range(graph_ncols):
                    ...
                    a.set_ylabel(x_axis_variant_labels[graph_row])
                    tp.xlabel = "Number of DOFs in cluster"
                    tp.ylabel = x_axis_variant_labels[graph_row]
