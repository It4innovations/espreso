...
inline std::ostream& operator<<(std::ostream& os, const CubeSettings &s)
{
	...
	for (size_t f = 0; f < 6; f++) {
		if (s.fillCondition[f].find(DIRICHLET_X)->second) {
			os << "DIRICHLET_X: " << s.boundaryCondition[f].find(DIRICHLET_X)->second << "\n";
		}
		if (s.fillCondition[f].find(DIRICHLET_Y)->second) {
			os << "DIRICHLET_Y: " << s.boundaryCondition[f].find(DIRICHLET_Y)->second << "\n";
		}
		if (s.fillCondition[f].find(DIRICHLET_Z)->second) {
			os << "DIRICHLET_Z: " << s.boundaryCondition[f].find(DIRICHLET_Z)->second << "\n";
		}
		if (s.fillCondition[f].find(FORCES_X)->second) {
			os << "FORCES_X: " << s.boundaryCondition[f].find(FORCES_X)->second << "\n";
		}
		if (s.fillCondition[f].find(FORCES_Y)->second) {
			os << "FORCES_Y: " << s.boundaryCondition[f].find(FORCES_Y)->second << "\n";
		}
		if (s.fillCondition[f].find(FORCES_Z)->second) {
			os << "FORCES_Z: " << s.boundaryCondition[f].find(FORCES_Z)->second << "\n";
		}
	}
	return os;
}
