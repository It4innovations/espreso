	...
	for (size_t z = 0; z < _settings.clusters[2]; z++) {
		for (size_t y = 0; y < _settings.clusters[1]; y++) {
			for (size_t x = 0; x < _settings.clusters[0]; x++) {
				if (x < _settings.layers ||
					x >= _settings.clusters[0] - _settings.layers ||
					y < _settings.layers ||
					y >= _settings.clusters[1] - _settings.layers ||
					z < _settings.layers ||
					z >= _settings.clusters[2] - _settings.layers) {
...
template<class TElement>
void SphereGenerator<TElement>::fillCluster(int rank, size_t cluster[])
{
	eslocal index = 0, i = 0;
	for (size_t z = 0; z < _settings.clusters[2]; z++) {
		for (size_t y = 0; y < _settings.clusters[1]; y++) {
			for (size_t x = 0; x < _settings.clusters[0]; x++) {
				if (_clusterMap[index++] == rank) {
					cluster[0] = x;
					cluster[1] = y;
					cluster[2] = z;
					return;
...
template <class TElement>
void SphereGenerator<TElement>::mesh(mesh::Mesh &mesh, const size_t cluster[])
{
	...
	for (eslocal i = 0; i < 3; i++) {
		step[i] = 2.0 / ((nodes[i] - 1) * _settings.clusters[i]);
	}

	esglobal offset[3] = { 0, 0, 0 };

	mesh::Point point, corner;
	esglobal max;
	...
	for (esglobal z = cs[2]; z <= ce[2]; z++) {
		...
		for (esglobal y = cs[1]; y <= ce[1]; y++) {
			...
			for (esglobal x = cs[0]; x <= ce[0]; x++) {
				...
				if (std::fabs(x - mid[0]) >= std::fabs(y - mid[1]) && std::fabs(x - mid[0]) >= std::fabs(z - mid[2])) {
					max = x;
				}
				if (std::fabs(y - mid[1]) >= std::fabs(x - mid[0]) && std::fabs(y - mid[1]) >= std::fabs(z - mid[2])) {
					max = y;
				}
				if (std::fabs(z - mid[2]) >= std::fabs(y - mid[1]) && std::fabs(z - mid[2]) >= std::fabs(x - mid[0])) {
					max = z;
				}
				corner.x = max * step[0] - 1;
				corner.y = max * step[1] - 1;
				corner.z = max * step[2] - 1;
				point.x = x * step[0] - 1;
				point.y = y * step[1] - 1;
				point.z = z * step[2] - 1;
				if (point.length() != 0) {
					point = (point / point.length()) * corner.length();
				}
				coordinates.add(point, local++, global);
			}
		}
	}
...
void SphereGenerator<TElement>::fixInnerSurface(mesh::Mesh &mesh, const size_t cluster[])
{
	...
	if (cluster[0] == _settings.layers - 1 &&
		cluster[1] >= _settings.layers &&
		cluster[1] <= _settings.clusters[1] - _settings.layers &&
		cluster[2] >= _settings.layers &&
		cluster[2] <= _settings.clusters[2] - _settings.layers) {
	...
	if (cluster[0] == _settings.clusters[0] - _settings.layers &&
		cluster[1] >= _settings.layers &&
		cluster[1] <= _settings.clusters[1] - _settings.layers &&
		cluster[2] >= _settings.layers &&
		cluster[2] <= _settings.clusters[2] - _settings.layers) {
	...
	if (cluster[1] == _settings.layers - 1 &&
		cluster[0] >= _settings.layers &&
		cluster[0] <= _settings.clusters[0] - _settings.layers &&
		cluster[2] >= _settings.layers &&
		cluster[2] <= _settings.clusters[2] - _settings.layers) {


		eslocal y = nodes[1] - 1;
		eslocal index = (y - 1) * nodes[0];
		for (eslocal z = 0; z < nodes[2]; z++) {
	...
	if (cluster[1] == _settings.clusters[1] - _settings.layers &&
		cluster[0] >= _settings.layers &&
		cluster[0] <= _settings.clusters[0] - _settings.layers &&
		cluster[2] >= _settings.layers &&
		cluster[2] <= _settings.clusters[2] - _settings.layers) {
	...
	if (cluster[2] == _settings.layers - 1 &&
		cluster[0] >= _settings.layers &&
		cluster[0] <= _settings.clusters[0] - _settings.layers &&
		cluster[1] >= _settings.layers &&
		cluster[1] <= _settings.clusters[1] - _settings.layers) {
		...
		eslocal z = nodes[2] - 1;
		eslocal index = (z - 1) * nodes[0] * nodes[1];
		for (eslocal y = 0; y < nodes[1]; y++) {
			for (eslocal x = 0; x < nodes[0]; x++) {
				if (e.addPoint(x, y, z)) {
					dirichlet_z[e.projectPoint(index)] = 0;
	...
	if (cluster[2] == _settings.clusters[2] - _settings.layers &&
		cluster[0] >= _settings.layers &&
		cluster[0] <= _settings.clusters[0] - _settings.layers &&
		cluster[1] >= _settings.layers &&
		cluster[1] <= _settings.clusters[1] - _settings.layers) {
...
template <class TElement>
void SphereGenerator<TElement>::fillGlobalBoundaries(mesh::Boundaries &boundaries, const size_t cluster[])
{
	esglobal gNodes[3];
	Utils<TElement>::globalNodesCount(_settings, gNodes);
	eslocal cNodes[3];
		...
		for (esglobal y = cs[1]; y <= ce[1]; y++) {
			...
			for (esglobal x = cs[0]; x <= ce[0]; x++) {
				...
				for (int i = 0; i < 8; i++) {
					...
					if (_clusterMap[tmp] >= 0) {
						boundaries[index].insert(_clusterMap[tmp]);
					}
				}
				index++;
			}
...
template <class TElement>
void SphereGenerator<TElement>::setFixPoints(mesh::Mesh &mesh)
{
...
template <class TElement>
void SphereGenerator<TElement>::setCorners(
