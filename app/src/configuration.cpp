...
void Configuration::fillDefaultValues()
{
	...
	_parameters[PMCUBE_SHAPE] = new IntegerParameter("PMCUBE_SHAPE", "The type of generated shape in PermonCuve.", 0);
	...
	_parameters[PMCUBE_SPHERE_LAYERS] = new IntegerParameter("PMCUBE_SPHERE_LAYERS", "Number of layers in sphere.", 1);
	_parameters[PMCUBE_SPHERE_INNER_RADIUS] = new DoubleParameter("PMCUBE_SPHERE_INNER_RADIUS", "Inner sphere radius.", 9);
	_parameters[PMCUBE_SPHERE_OUTER_RADIUS] = new DoubleParameter("PMCUBE_SPHERE_OUTER_RADIUS", "Outer sphere radius.", 12);
...
void Configuration::configure(int argc, char** argv)
{
	...
	for (size_t i = PMCUBE_SHAPE; i <= PMCUBE_ELEMENTS_Z; i++) {
		if (_parameters[MESH_FILE]->isSet()) {
			std::cout << "Warning: parameter " << _parameters[i]->name() << " is ignored. MESH_FILE has higher priority..\n";
			configured[i] = false;
