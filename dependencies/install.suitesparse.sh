...
then
    (
        ...
        mkdir -p build
        cd build
        cmake -DCMAKE_C_COMPILER=$1 -DENABLE_CUDA=false -DSUITESPARSE_ENABLE_PROJECTS="cholmod" -DCMAKE_INSTALL_PREFIX=${SUITESPARSE_ROOT}/$1 ..
        cmake --build . -j $(nproc)
        cmake --install .
    )
fi
...
export LIBRARY_PATH="${SUITESPARSE_ROOT}/$1/lib64:${LIBRARY_PATH}"
export LD_LIBRARY_PATH="${SUITESPARSE_ROOT}/$1/lib64:${LD_LIBRARY_PATH}"